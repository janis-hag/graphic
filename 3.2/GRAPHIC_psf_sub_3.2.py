...
if rank==0:
	...
	cube_list, dirlist, skipped=graphic_lib_320.create_megatable(dirlist, infolist, skipped, keys=header_keys, nici=nici, sphere=sphere, scexao=scexao, fit=fit)
	if dirlist==[]:
		for n in range(nprocs-1):
			comm.send("over",dest =n+1)
		sys.exit(1)
	...
	if l_max==0:
		for i in range(len(cube_list['info'])):
			...
			if not cube_list['info'][i][len(cube_list['info'][i])/2,4]==-1:
				...
				l=graphic_lib_320.get_max_dim(cube_list['info'][i][len(cube_list['info'][i])/2,4], cube_list['info'][i][len(cube_list['info'][i])/2,5], hdulist[0].header['NAXIS1'], cube_list['info'][i][:,11])
				if l>l_max: l_max=l
	...
	for c in range(len(cube_list['cube_filename'])): # Loop over the cubes
		...
		graphic_lib_320.iprint(interactive, "Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+"\n")
		...
		for f in range(len(cube_list['info'][c])): # Loop over the frames in the cube
			...
			graphic_lib_320.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c])))
			...
			if cube_list['info'][c][f][6]==-1:
				graphic_lib_320.dprint(d>1, "Skipping bad frame (c,f): "+str(c)+","+str(f)+", info: "+str(cube_list['info'][c][f]))
				continue
			...
			for n in range(-1,2*len(cube_list['cube_filename'])):
				...
				if cn<0:
					graphic_lib_320.dprint(d>3,"Wrapping (c,n,cn): "+str(c)+","+str(n)+","+str(cn))
					graphic_lib_320.dprint(d>1, "TooEarly")
					TooEarly=True
					continue
				elif cn+1>len(cube_list['cube_filename']):
					graphic_lib_320.dprint(d>3,"Wrapping (c,n,cn): "+str(c)+","+str(n)+","+str(cn))
					graphic_lib_320.dprint(d>1, "TooLate")
					TooLate=True
					continue
				...
				if np.min(cube_list['info'][cn][1:,10])>t0+ctmax-time_treshold:
					...
					TooLate=True
					graphic_lib_320.dprint(d>1, "TooLate: np.min(cube_list['info'][cn][1:,10])"+str(np.min(cube_list['info'][cn][1:,10]))+">"+str(t0)+'+'+str(ctmax)+'='+str(t0+ctmax)+'=t0+ctmax')
				elif np.max(cube_list['info'][cn][1:,10])<t0-ctmax+time_treshold:
					# Need to use [1:,10] as older cube-info had the first date in LST
					TooEarly=True
					graphic_lib_320.dprint(d>1, "TooEarly: np.max(cube_list['info'][cn][1:,10])"+str(np.max(cube_list['info'][cn][1:,10]))+"<"+str(t0)+'-'+str(ctmax)+'='+str(t0-ctmax)+'=t0-ctmax')
				...
				graphic_lib_320.dprint(d>2, "cube_list['info'][cn]"+str(cube_list['info'][cn]))
				graphic_lib_320.dprint(d>3,cube_list['info'][cn][:,11])
				...
				graphic_lib_320.dprint(d>0, "Valid: "+str(cube_list['cube_filename'][cn])+', '+str(valid)+', '+str(len(valid)))
			...
			if stack == None or sum(len(i) for i in valid.itervalues())==0:
				...
				graphic_lib_320.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c]))+" calculating median. Kept "+str(si)+" out of "+str(sum(len(i) for i in valid.itervalues()))+" valid frames.")


				tb=MPI.Wtime()
				t0_trans=MPI.Wtime()
				if d>0:
					graphic_lib_320.save_fits('temp_psf_stack.fits',stack,hdr=hdr, backend='pyfits')
				...
				if args.stat==True:
					graphic_lib_320.iprint(interactive, "\n STAT: Data download took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib_320.humanize_time(MPI.Wtime()-t0_trans))
					graphic_lib_320.iprint(interactive, "\n STAT: Median calculation took: "+str(MPI.Wtime()-tb)+" s = "+graphic_lib_320.humanize_time(MPI.Wtime()-tb))
		...
		if final_cube==None:
			...
			graphic_lib_320.iprint(interactive, "Remaining time: "+graphic_lib_320.humanize_time((MPI.Wtime()-t_init)*(len(cube_list['cube_filename'])-c)/(c-skipped+1))+"\n")
			...
			if not (stack==None or len(stack.shape)<3):
				...
				for fn in range(bigstack.shape[0]):
					...
					if nofft==True: # Use interpolation
						...
						if 2*l_max<stack_shape[1]:
							smallstack[fn]=graphic_lib_320.fft_shift(bigstack[fn], stack_shape[1]/2.-info_stack[s+fn,4], stack_shape[2]/2.-info_stack[s+fn,5])[
								bigstack.shape[1]/2-l_max:l_max+bigstack.shape[1]/2,bigstack.shape[2]/2-l_max:l_max+bigstack.shape[2]/2]
						else:
							bigstack[fn]=graphic_lib_320.fft_shift(bigstack[fn], stack_shape[1]/2.-info_stack[s+fn,4], stack_shape[2]/2.-info_stack[s+fn,5])
