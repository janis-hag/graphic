...
import argparse
import graphic_lib_320 as graphic_lib
import random
...
def read_recentre(rcn, rcubes, rcube_list, l_max):
	...
	if "GC RECENTER" in rhdr.keys():
		graphic_lib.iprint(interactive, '\r\r\r Already recentred: '+str(rcube_list['cube_filename'][rcn]))
		return rcubes, t0_trans
	# Send the cube to be recentreed
	graphic_lib.iprint(interactive, '\r\r\r Recentring '+str(rcube_list['cube_filename'][rcn]))

	comm.bcast("recentre",root=0)
	comm.bcast(l_max,root=0)
	comm.bcast(rcube_list['info'][rcn],root=0)
	graphic_lib.send_frames_async(rcubes[rcn])
	## rcubes.pop(rcn)
	rcubes[rcn]=None
	#rcube_list['info'][rcn]=None

	if args.stat==True:
		print("\n STAT: Data upload took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-t0_trans))
		t0_trans=MPI.Wtime()
	...
	for n in range(nprocs-1):
		...
		graphic_lib.iprint(interactive, '\r\r\r Recentreed data from '+str(n+1)+' received									   =>')
...
if rank==0:
	...
	cube_list, dirlist, skipped=graphic_lib.create_megatable(dirlist, infolist, skipped, keys=header_keys, nici=nici, sphere=sphere, scexao=scexao, fit=fit)
	if dirlist==[]:
		for n in range(nprocs-1):
			comm.send("over",dest =n+1)
		sys.exit(1)
	...
	if l_max==0:
		for i in range(len(cube_list['info'])):
			...
			if not cube_list['info'][i][len(cube_list['info'][i])/2,4]==-1:
				...
				l=graphic_lib.get_max_dim(cube_list['info'][i][len(cube_list['info'][i])/2,4], cube_list['info'][i][len(cube_list['info'][i])/2,5], hdulist[0].header['NAXIS1'], cube_list['info'][i][:,11])
				if l>l_max: l_max=l
		graphic_lib.dprint(d>1, 'l_max: '+str(l_max))
	...
	for c in range(len(cube_list['cube_filename'])): # Loop over the cubes
		...
		graphic_lib.iprint(interactive, "Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+"\n")
		...
		for f in range(len(cube_list['info'][c])): # Loop over the frames in the cube
			...
			graphic_lib.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c])))
			...
			if cube_list['info'][c][f][6]==-1:
				graphic_lib.dprint(d>1, "Skipping bad frame (c,f): "+str(c)+","+str(f)+", info: "+str(cube_list['info'][c][f]))
				continue
			...
			for n in range(-1,2*len(cube_list['cube_filename'])):
				...
				if cn<0:
					graphic_lib.dprint(d>3,"Wrapping (c,n,cn): "+str(c)+","+str(n)+","+str(cn))
					graphic_lib.dprint(d>1, "TooEarly")
					TooEarly=True
					continue
				elif cn+1>len(cube_list['cube_filename']):
					graphic_lib.dprint(d>3,"Wrapping (c,n,cn): "+str(c)+","+str(n)+","+str(cn))
					graphic_lib.dprint(d>1, "TooLate")
					TooLate=True
					continue
				...
				if np.min(cube_list['info'][cn][1:,10])>t0+ctmax-time_treshold:
					...
					TooLate=True
					graphic_lib.dprint(d>1, "TooLate: np.min(cube_list['info'][cn][1:,10])"+str(np.min(cube_list['info'][cn][1:,10]))+">"+str(t0)+'+'+str(ctmax)+'='+str(t0+ctmax)+'=t0+ctmax')
				elif np.max(cube_list['info'][cn][1:,10])<t0-ctmax+time_treshold:
					# Need to use [1:,10] as older cube-info had the first date in LST
					TooEarly=True
					graphic_lib.dprint(d>1, "TooEarly: np.max(cube_list['info'][cn][1:,10])"+str(np.max(cube_list['info'][cn][1:,10]))+"<"+str(t0)+'-'+str(ctmax)+'='+str(t0-ctmax)+'=t0-ctmax')
				...
				graphic_lib.dprint(d>2, "cube_list['info'][cn]"+str(cube_list['info'][cn]))
				graphic_lib.dprint(d>3,cube_list['info'][cn][:,11])
				...
				graphic_lib.dprint(d>0, "Valid: "+str(cube_list['cube_filename'][cn])+', '+str(valid)+', '+str(len(valid)))
				if len(valid[(n+1,cn)])==0: # Remove empty list
					...
					if not cn in cubes.keys(): # Check if cube already loaded
						if centred: # Check if cube already centred (quicklook)
							...
							# ACC Nov 2015: be careful with the array indices when putting one arbitrary sized array into another arbitrary sized array
						...
						if args.stat==True:
							print("\n STAT: Data download took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-t0_trans))
							print("\n STAT: Recentreing took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-tb))
			...
			if not len(valid.keys())==0:
				...
				## if stack is None:
					## graphic_lib.dprint(d>19, "stack: "+str(stack)+", cube_list['info'][cn]: "+str(cube_list['info'][cn]))
					## # Create frame stack
			...
			if args.stat==True:
				print("\n STAT: Stack preparation took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-tb))

			#graphic_lib.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c]))+" recentreing stack. Kept "+str(stack.shape[0])+" out of "+str(valid_count)"+ frames.")


			if stack is None or sum(len(i) for i in valid.itervalues())==0:
				empty_frame=empty_frame+1
				graphic_lib.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c]))+" .... no frames found to generate PSF. Valid_cubes="+str(valid_cubes)+" si="+str(si))
			else:
				graphic_lib.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c]))+" calculating median. Kept "+str(si)+" out of "+str(sum(len(i) for i in valid.itervalues()))+" valid frames.")


				tb=MPI.Wtime()
				t0_trans=MPI.Wtime()
				if d>0:
					graphic_lib.save_fits('temp_psf_stack.fits',stack,hdr=hdr, backend='pyfits')

				# Send chunks to get median (cut along x axis)
				comm.bcast("median",root=0)
				graphic_lib.send_chunks(stack,d)
				if args.stat==True:
					graphic_lib.iprint(interactive, "\n STAT: Data upload took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-t0_trans)+"\n")
				...
				for n in range(nprocs-1):
					...
					graphic_lib.iprint(interactive, '\r\r\r Median processed data from '+str(n+1)+' received									 =>')
				...
				if args.stat==True:
					graphic_lib.iprint(interactive, "\n STAT: Data download took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-t0_trans))
					graphic_lib.iprint(interactive, "\n STAT: Median calculation took: "+str(MPI.Wtime()-tb)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-tb))
				...
				if args.stat==True:
					print("\n STAT: Frame processing took: "+str(MPI.Wtime()-t0_trans)+" s = "+graphic_lib.humanize_time(MPI.Wtime()-t0_frame))
				graphic_lib.dprint(d==-31 and not new_info is None, str(f)+" new_info "+str(len(new_info)))
				graphic_lib.dprint(d==-31 and not final_cube is None, "final_cube.shape: "+str(final_cube.shape))
				graphic_lib.dprint(d==-31 and final_cube is None, "final_cube: "+str(final_cube))
		...
		if final_cube is None:
			...
			## graphic_lib.save_fits(psf_sub_filename, final_cube, hdr=hdr, backend='pyfits')
			graphic_lib.save_fits(psf_sub_filename, final_cube, hdr=hdr, backend='pyfits')
			...
			graphic_lib.write_array2rdb(info_dir+os.sep+info_filename,new_info,header_keys)

			graphic_lib.iprint(interactive, "\n Saved: {name} .\n Processed in {human_time} at {rate:.2f} MB/s \n {cubes_in_mem} loaded in memory \n"
							 .format(name=psf_sub_filename, human_time=graphic_lib.humanize_time(MPI.Wtime()-t0_cube) ,
									 rate=os.path.getsize(psf_sub_filename)/(1048576*(MPI.Wtime()-t0_cube)),
									 cubes_in_mem=len(cubes.keys())))
			graphic_lib.iprint(interactive, "Remaining time: "+graphic_lib.humanize_time((MPI.Wtime()-t_init)*(len(cube_list['cube_filename'])-c)/(c-skipped+1))+"\n")

		del final_cube


	print("\n Program finished, killing all the slaves...")
	print("Total time: "+str(MPI.Wtime()-t_init)+" s = "+graphic_lib.humanize_time((MPI.Wtime()-t_init)))
	...
	graphic_lib.write_log((MPI.Wtime()-t_init),log_file)
	if nici:
		...
		if todo=="median":
			...
			if not (stack is None or len(stack.shape)<3):
				...
				for fn in range(bigstack.shape[0]):
					graphic_lib.dprint(d>2, "recentreing frame: "+str(fn)+" with shape: "+str(bigstack[fn].shape))
					...
					if nofft==True: # Use interpolation
						...
						if 2*l_max<stack_shape[1]:
							smallstack[fn]=graphic_lib.fft_shift(bigstack[fn], stack_shape[1]/2.-info_stack[s+fn,4], stack_shape[2]/2.-info_stack[s+fn,5])[
								bigstack.shape[1]/2-l_max:l_max+bigstack.shape[1]/2,bigstack.shape[2]/2-l_max:l_max+bigstack.shape[2]/2]
						else:
							bigstack[fn]=graphic_lib.fft_shift(bigstack[fn], stack_shape[1]/2.-info_stack[s+fn,4], stack_shape[2]/2.-info_stack[s+fn,5])
				...
				if 2*l_max<stack_shape[1]:
					...
					del smallstack
				graphic_lib.dprint(d>2, "Sending back bigstack, shape="+str(bigstack.shape))
