...
if rank==0:
	...
	dirlist=graphic_lib_300.create_dirlist(pattern)
	if dirlist==None:
		print("No files found. Check --pattern option!")
		comm.bcast("over",root=0)
		sys.exit(1)

	infolist=graphic_lib_300.create_dirlist(info_dir+os.sep+info_pattern, extension='.rdb')
	...
	for step in range(steps):
		...
		graphic_lib_300.send_dirlist_slaves(dirlist)
		#stack the result
		...
		if args.save_stack:
			graphic_lib_300.save_fits("derot_stack_"+step_filename, stack, hdr=hdr , backend='pyfits' )

		print(' Step ['+str(step+1)+'/'+str(steps)+'] of median calculation')
		comm.bcast("median", root=0)
		graphic_lib_300.send_chunks(stack,d)
		...
		graphic_lib_300.save_fits(step_filename, med_tot, hdr=hdr, backend='pyfits' )
		print("Step time: "+graphic_lib_300.humanize_time((MPI.Wtime()-t0_step)))
	...
	if "GC RMIN" in hdr.keys():
		...
		fwhm=bottleneck.median(cube_list['info'][0][np.where(cube_list['info'][0][:,7]>0),7])
	if nomask:
		med_nomask=med_tot.copy()
	med_tot=graphic_lib_300.mask_center(med_tot, fwhm/2., med_tot.shape[0]/2., med_tot.shape[0]/2. )

	print("Saving: "+str(finalname))
	graphic_lib_300.save_fits(finalname, med_tot, hdr=hdr, backend='pyfits' )
	if nomask:
		graphic_lib_300.save_fits('nomask_'+finalname, med_nomask, hdr=hdr, backend='pyfits' )
	comm.bcast("over",root=0)
	MPI.Finalize()
	print("Total time: "+str(MPI.Wtime()-t_init)+" s = "+graphic_lib_300.humanize_time((MPI.Wtime()-t_init)))

	## log_file=log_file+"_"+hdr['HIERARCH ESO OBS TARG NAME']+"_"+str(__version__)+".log"
	graphic_lib_300.write_log((MPI.Wtime()-t_init),log_file)
	sys.exit(0)
	...
	while not todo=="over":
		if todo=="derotate":
			...
			for i in range(len(dirlist)):
				## graphic_lib_300.iprint(interactive, '\r\r\r '+str(rank)+': Derotating cube '+str(i+1)+' of '+str(len(dirlist))+' : '+str(dirlist[i]))
				graphic_lib_300.iprint(interactive, '\r\r\r '+str(rank)+': Derotating cube '+str(i+1)+' of '+str(len(dirlist))+' : '+str(dirlist[i])+'\n')
				...
				## if s_cube.shape[0]==1:
					## graphic_lib_300.dprint(d>1, 'Skipping corrupt frame. cube '+str(cn)+', frame '+str(fn))
					## continue
				for fn in range(s_cube.shape[0]):
					if cube_list['info'][cn][fn,5]==-1: # Invalid frame, skip
						graphic_lib_300.dprint(d>1, 'Skipping invalid frame, no PSf found: cube '+str(cn)+', frame '+str(fn))
						s_cube=s_cube[1:]
						continue
					elif interpolate:
					...
					## elif not naxis1==0:
						...
						rs_cube[fn]=graphic_lib_300.fft_3shear_rotate_pad(
					...
					if False:
						...
						## rs_cube[fn]=graphic_lib_300.fft_3shear_rotate_pad(s_cube[0],p0-cube_list['info'][cn][fn,11], x1=-1, pad=2)[:,start:end]
						rs_cube[fn]=graphic_lib_300.fft_3shear_rotate_pad(s_cube[0],p0-cube_list['info'][cn][fn,11], pad=2)[:,start:end]
						## temp=graphic_lib_300.fft_3shear_rotate_pad(s_cube[0],p0-cube_list['info'][cn][fn,11], pad=2)
						## if rank==2:
						...
						s_cube=s_cube[1:]
				if d>2:
					graphic_lib_300.save_fits('rs_cube_'+str(rank)+'_'+dirlist[i], rs_cube, hdr=hdulist_s[0].header , backend='pyfits' )
			...
			graphic_lib_300.iprint(interactive, "\r\r\r Process "+str(rank)+" of "+str(nprocs-1)+" calculating median.")

			start_col=comm.recv(source=0)
			my_cube=comm.recv(source=0)
			my_cube=bottleneck.nanmedian(my_cube,axis=0)
