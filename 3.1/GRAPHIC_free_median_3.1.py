...
parser = argparse.ArgumentParser(description='Creates a median from all the frames.')
...
parser.add_argument('--sky_dir', action="store", dest="sky_dir",  default='.', help='Sky directory')
...
if rank==0:
	...
	t_init=MPI.Wtime()
	dirlist=graphic_lib_310.create_dirlist(sky_dir+os.sep+pattern,target_pattern=target_pattern+"_")
	...
	for c in range(len(dirlist)):
		...
		sys.stdout.write("Processing cube ["+str(c+1)+"/"+str(len(dirlist))+"]: "+str(dirlist[c])+"\n")
		sys.stdout.flush()
		s=1
		restart=False
		total_frames=0
	...
	if args.stat==True:
		print("\n STAT: Stack preparation took: "+humanize_time(MPI.Wtime()-tb))
		tb=MPI.Wtime()
		t0_trans=MPI.Wtime()
	...
	for n in range(nprocs-1):
		...
		sys.stdout.write('\r\r\r Median processed data from '+str(n+1)+' received								 =>')
		sys.stdout.flush()
	...
	if args.stat==True:
		print("\n STAT: Data download took: "+humanize_time(MPI.Wtime()-t0_trans))
		print("\n STAT: Median calculation took: "+humanize_time(MPI.Wtime()-tb))
	...
	graphic_lib_310.save_fits(median_filename, med, target_dir=target_dir, hdr=hdr, backend='pyfits')

	sys.stdout.write("\n Saved: {name} .\n Processed in {human_time} at {rate:.2f} MB/s \n"
						 .format(name=median_filename, human_time=graphic_lib_310.humanize_time(MPI.Wtime()-t0_cube) ,
								 rate=os.path.getsize(target_dir+os.sep+median_filename)/(1048576*(MPI.Wtime()-t0_cube))))
	sys.stdout.flush()
