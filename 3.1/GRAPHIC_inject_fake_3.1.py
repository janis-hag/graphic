...
header_keys=['frame_number', 'psf_barycenter_x', 'psf_barycenter_y',
...
if rank==0:
	dirlist=graphic_lib_310.create_dirlist(pattern,target_pattern=target_pattern+"_")
	...
	if args.info:
		...
		if infolist==None:
			...
			sys.exit(1)
		cube_list, complete_dirlist=graphic_lib_310.create_megatable(complete_dirlist,infolist,keys=header_keys,nici=nici,fit=fit)
...
for i in range(len(dirlist)):
	...
	for frame in range(cube.shape[0]):
		if not args.info:
			print("Debug: no info")
			if nici:
				cube[frame]=graphic_lib_310.inject_FP_nici(cube[frame], sep_vect, prim_adu, dm, header, alpha=0, x0=0,y0=0, r_tel_prim=8.2, r_tel_sec=1.2, noise=args.noise, pad=2)
			else:
				cube[frame]=graphic_lib_310.inject_FP(cube[frame], sep_vect, prim_adu, dm, header, alpha=0, x0=0,y0=0, r_tel_prim=8.2, r_tel_sec=1.2, noise=args.noise, pad=2)
		elif all_info[frame,5]==-1:
			continue
		if nici:
			cube[frame]=graphic_lib_310.inject_FP_nici(cube[frame], sep_vect, prim_adu, dm, header, alpha=all_info[frame,11], x0=all_info[frame,4],y0=all_info[frame,5], r_tel_prim=8.2, r_tel_sec=1.0, noise=args.noise)
		else:
			## cube[frame]=graphic_lib_310.inject_FP(cube[frame], sep_vect, prim_adu, dm, header, alpha=all_info[frame,11], x0=all_info[frame,4]-cube.shape[1]/2,y0=all_info[frame,5]-cube.shape[2]/2, r_tel_prim=8.2, r_tel_sec=1.16, noise=args.noise)
			cube[frame]=graphic_lib_310.inject_FP(cube[frame], sep_vect, prim_adu, dm, header, alpha=all_info[frame,11], x0=all_info[frame,4],y0=all_info[frame,5], r_tel_prim=8.2, r_tel_sec=1.16, noise=args.noise, pad=2)
	...
	graphic_lib_310.save_fits( targetfile, hdulist )
...
if rank==0:
	...
	graphic_lib_310.write_log((MPI.Wtime()-t_init),log_file, comments=None)
sys.exit(0)
