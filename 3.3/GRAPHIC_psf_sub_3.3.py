...
parser.add_argument('--n_fwhm', action="store", dest="n_fwhm", type=float,
                    default=0.75, help='Number of FWHM displacements')
parser.add_argument(
        '--dfwhm', action="store", dest="dfwhm", type=float, default=0.25,
        help='Number of FWHM displacements between PSF frames, useful for disks'
)
parser.add_argument('--rmin', action="store", dest="rmin", type=float,
                    default=5, help='The minimal size in pixels to consider')
parser.add_argument(
        '--tmax', action="store", dest="tmax", type=float, default=300,
        help='The maximum time difference in second between the frames')
parser.add_argument(
        '--fmax', action="store", dest="fmax", type=int, default=300,
        help='The maximum number of frames to use to generate the PSF')
parser.add_argument(
        '--fwhm', action="store", dest="fwhm", type=float, default=np.NaN,
        help='Give a custom PSF FWHM value, overriding the fitted value.')
parser.add_argument(
        '--lmax', action="store", dest="l_max", type=float, default=0, help=
        'Shape of the final image. If not specified it will be calculated to fit all the images.'
)
parser.add_argument('--ndither', action="store", dest="ndither", type=int,
                    default=4, help='Number of dithering steps.')
parser.add_argument('--log_file', action="store", dest="log_file",
                    default='GRAPHIC', help='Log filename')
parser.add_argument('--max_in_mem', action="store", dest="max_in_mem",
                    default=None, help='Maximum number of cubes in memory')
parser.add_argument(
        '-centred', dest='centred', action='store_const', const=True,
        default=False,
        help='Use this switch if the frames don\'t need to be recentred.')
parser.add_argument('-nofft', dest='nofft', action='store_const', const=True,
                    default=False,
...
parser.add_argument('-disk', dest='disk', action='store_const', const=True,
                    default=False, help='Switch to use disk optimisations')
parser.add_argument('-interactive', dest='interactive', action='store_const',
                    const=True, default=False,
                    help='Switch to set execution to interactive mode')
parser.add_argument('-nomjd', dest='mjd', action='store_const', const=False,
                    default=True, help='Switch to use LST instead of MJD dates')
parser.add_argument(
        '-median', dest='combine', action='store_const', const='median',
        default='mean',
...
def read_recentre(rcn, rcubes, rcube_list, l_max):
    ...
    if "GC RECENTER" in rhdr.keys():
        graphic_nompi_lib.iprint(
                interactive, '\r\r\r Already recentred: ' +
                str(rcube_list['cube_filename'][rcn]))
        return rcubes, t0_trans
    # Send the cube to be recentreed
    graphic_nompi_lib.iprint(
            interactive,
            '\r\r\r Recentring ' + str(rcube_list['cube_filename'][rcn]))
    ...
    if args.stat == True:
        print("\n STAT: Data upload took: " + str(MPI.Wtime() - t0_trans) +
              " s = " + graphic_nompi_lib.humanize_time(MPI.Wtime() - t0_trans))
        t0_trans = MPI.Wtime()
    ...
    for n in range(nprocs - 1):
        ...
        graphic_nompi_lib.iprint(
                interactive, '\r\r\r Recentreed data from ' + str(n + 1) +
                ' received                                       =>')
...
if rank == 0:
    ...
    dirlist = graphic_nompi_lib.create_dirlist(pattern, target_dir='.',
                                               extension='.fits')
    ...
    infolist = graphic_nompi_lib.create_dirlist(
            info_dir + os.sep + '*' + info_pattern, target_dir=info_dir,
            extension='.rdb')
    ...
    cube_list, dirlist, skipped = graphic_nompi_lib.create_megatable(
            dirlist, infolist, skipped, keys=header_keys, nici=nici,
            sphere=sphere, scexao=scexao, fit=fit)
    if dirlist == []:
        for n in range(nprocs - 1):
            comm.send("over", dest=n + 1)
        sys.exit(1)
    ...
    for c in range(len(cube_list['cube_filename'])):  # Loop over the cubes
        ...
        if np.isnan(fwhm):
            fwhm = bottleneck.median(cube_list['info'][c][np.where(
                    cube_list['info'][c][:, 7] > 0), 7])

        if l_max == 0:
            if disk:
                psf_sub_filename = target_pattern + 'disk_' + str(
                        fwhm) + '_' + str(n_fwhm) + "_" + str(
                                dfwhm) + "_" + str(rmin) + "_" + str(
                                        tmax
                                ) + "_" + cube_list['cube_filename'][c]
            else:
                psf_sub_filename = target_pattern + str(fwhm) + '_' + str(
                        n_fwhm) + "_" + str(rmin) + "_" + str(
                                tmax) + "_" + cube_list['cube_filename'][c]
        else:
            if disk:
                psf_sub_filename = target_pattern + '_disk_' + str(
                        fwhm
                ) + '_' + str(n_fwhm) + "_" + str(dfwhm) + "_" + str(
                        rmin) + "_" + str(tmax) + "_" + str(np.ceil(
                                2 *
                                l_max)) + '_' + cube_list['cube_filename'][c]
            else:
                psf_sub_filename = target_pattern + str(fwhm) + '_' + str(
                        n_fwhm) + "_" + str(rmin) + "_" + str(tmax) + "_" + str(
                                np.ceil(2 * l_max)
                        ) + '_' + cube_list['cube_filename'][c]
        ...
        graphic_nompi_lib.iprint(
                interactive, "Processing cube [" + str(c + 1) + "/" +
                str(len(cube_list['cube_filename'])) + "]: " +
                str(cube_list['cube_filename'][c]) + "\n")
        ...
        for f in range(len(
                cube_list['info'][c])):  # Loop over the frames in the cube
            t0_frame = MPI.Wtime()

            graphic_nompi_lib.iprint(
                    interactive, "\r\r\r Processing cube [" + str(c + 1) + "/" +
                    str(len(cube_list['cube_filename'])) + "]: " +
                    str(cube_list['cube_filename'][c]) + ", frame " +
                    str(f + 1) + "/" + str(len(cube_list['info'][c])))
            ...
            if cube_list['info'][c][f][6] == -1:
                graphic_mpi_lib.dprint(
                        d > 1, "Skipping bad frame (c,f): " + str(c) + "," +
                        str(f) + ", info: " + str(cube_list['info'][c][f]))
                continue
            ...
            cube_timespan = np.abs(cube_list['info'][c][0][10] -
                                   cube_list['info'][c][-1][10])
            valid_count = 0
            valid_cubes = 0
            valid = {}

            if not max_in_mem is None and len(cubes.keys()) > max_in_mem:
                print('Too many cubes loaded in memory, temporarly adapting ctmax, removing '
                      + str(cube_timespan))
                time_treshold = time_treshold + cube_timespan
            elif time_treshold > 0:
                print('Less cubes loaded in memory, re-adapting ctmax, adding ' +
                      str(cube_timespan))
                time_treshold = time_treshold - cube_timespan

            if len(cubes.keys()) > 0:
                for cn in cubes.keys():
                    if np.min(np.abs(cube_list['info'][cn][:, 10] -
                                     t0)) > ctmax:
                        # pop this cube from list, since it's not needed anymore
                        cubes.pop(cn)
                        graphic_nompi_lib.iprint(
                                interactive, "\n Removing cube " + str(cn) +
                                " from stored cubes.")
            ...
            for n in range(-1, 2 * len(cube_list['cube_filename'])):
                ...
                if np.min(
                    ...
                    graphic_mpi_lib.dprint(
                            d > 1,
                            "TooLate: np.min(cube_list['info'][cn][0:,10])" +
                            str(np.min(cube_list['info'][cn][0:, 10])) + ">" +
                            str(t0) + '+' + str(ctmax) + '=' + str(t0 + ctmax) +
                            '=t0+ctmax')
                    ...
                    graphic_mpi_lib.dprint(
                            d > 1,
                            "TooEarly: np.max(cube_list['info'][cn][0:,10])" +
                            str(np.max(cube_list['info'][cn][0:, 10])) + "<" +
                            str(t0) + '-' + str(ctmax) + '=' + str(t0 - ctmax) +
                            '=t0-ctmax')
                ...
                if d > 0:
                    print("")
                    print("debug 3: stacking condition: alpha=" + str(alpha) +
                          ", p0=" + str(p0) + ", t0=" + str(t0) + ", ctmax=" +
                          str(ctmax))
                    print("cn: " + str(cn))
                    if d > 1:
                        print("angle-p0: " +
                              str(cube_list['info'][cn][:, 11] - p0))
                        print("time-t0: " +
                              str(cube_list['info'][cn][:, 10] - t0))
                    else:
                        print("angle>alpha: " + str(
                                np.where(
                                        np.abs(cube_list['info'][cn][:, 11] -
                                               p0) > alpha)))
                        print("time<ctmax: " + str(
                                np.where(
                                        np.abs(cube_list['info'][cn][:, 10] -
                                               t0) < ctmax)))

                graphic_mpi_lib.dprint(
                        d > 2,
                        "cube_list['info'][cn]" + str(cube_list['info'][cn]))
                ...
                ############
                valid[(n + 1, cn)] = np.where(
                        (np.abs(cube_list['info'][cn][:, 11] - p0) > alpha) &
                        (np.abs(cube_list['info'][cn][:, 10] - t0) < ctmax) &
                        np.not_equal(cube_list['info'][cn][:, 6], -1))[
                                0]  #(not cube_list['info'][cn][:,2]<0))]
                ...
                graphic_mpi_lib.dprint(
                        d > 0, "Valid: " + str(cube_list['cube_filename'][cn]) +
                        ', ' + str(valid) + ', ' + str(len(valid)))
                if len(valid[(n + 1, cn)]) == 0:  # Remove empty list
                    ...
                    if not cn in cubes.keys():  # Check if cube already loaded
                        if centred:  # Check if cube already centred (quicklook)
                            ...
                            bigstack = np.ones((data.shape[0], l_max * 2,
                                                l_max * 2)) * np.NaN
                            ...
                            mindim = np.min(
                                    [l_max * 2, data.shape[1]]
                            )  # This ensures that l_max < data.shape[1] and l_max > data.shape[1] both work.
                            xstack_min = np.max([0., l_max - mindim // 2])
                            xstack_max = np.min([
                                    l_max * 2, l_max + mindim // 2
                            ])
                            ...
                            xdata_min = np.max([
                                    0., data.shape[1] // 2 - mindim // 2
                            ])
                            xdata_max = np.min(
                                    [data.shape[1], data.shape[1] // 2] +
                                    mindim // 2)
                            ydata_min = np.max([
                                    0., data.shape[2] // 2 - mindim // 2
                            ])
                            ydata_max = np.min(
                                    [data.shape[2], data.shape[2] // 2] +
                                    mindim // 2)
                            bigstack[:, xstack_min:xstack_max, ystack_min:
                                     ystack_max] = data[:, xdata_min:xdata_max,
                                                        ydata_min:ydata_max]
                            cubes[cn] = bigstack
                            del bigstack, data
                        else:  # Send cube for recentring
                            cubes, t0_trans = read_recentre(
                                    cn, cubes, cube_list, l_max)
                        if args.stat == True:
                            print("\n STAT: Data download took: " +
                                  str(MPI.Wtime() - t0_trans) + " s = " +
                                  graphic_nompi_lib.humanize_time(MPI.Wtime() -
                                                                  t0_trans))
                            print("\n STAT: Recentreing took: " +
                                  str(MPI.Wtime() - t0_trans) + " s = " +
                                  graphic_nompi_lib.humanize_time(MPI.Wtime() -
                                                                  tb))
            ...
            if not len(valid.keys()) == 0:
                ...
                for j, k in valid.keys():
                    if len(valid[(j, k)]) < sample:
                        stack[si:si + len(valid[(j, k)])] = cubes[k][valid[(j,
                                                                            k)]]
                        si += len(valid[(j, k)])
                        cube_count += 1
                    else:
                        # Could add PCA step here
                        stack[si:si + sample] = cubes[k][random.sample(
                                valid[(j, k)], sample)]
                        si += sample
            ...
            if stack is None or sum(len(i) for i in valid.itervalues()) == 0:
                ...
                graphic_nompi_lib.iprint(
                        ...
                        str(cube_list['cube_filename'][c]) + ", frame " +
                        str(f + 1) + "/" + str(len(cube_list['info'][c])) +
                        " .... no frames found to generate PSF. Valid_cubes=" +
                        str(valid_cubes) + " si=" + str(si))
            else:
                graphic_nompi_lib.iprint(
                        interactive, "\r\r\r Processing cube [" + str(c + 1) +
                        "/" + str(len(cube_list['cube_filename'])) + "]: " +
                        str(cube_list['cube_filename'][c]) + ", frame " +
                        str(f + 1) + "/" + str(len(cube_list['info'][c])) +
                        " calculating " + combine + ". Kept " + str(si) +
                        " out of " +
                        str(sum(len(i) for i in valid.itervalues())) +
                        " valid frames.")

                tb = MPI.Wtime()
                t0_trans = MPI.Wtime()
                if d > 0:
                    graphic_nompi_lib.save_fits('temp_psf_stack.fits', stack,
                ...
                for n in range(nprocs - 1):
                    ...
                    graphic_nompi_lib.iprint(
                            interactive, '\r\r\r ' + combine +
                            ' processed data from ' + str(n + 1) +
                            ' received                                     =>')
                ...
                if final_cube is None:  # Check if a cube has already been started
                    ...
                    final_cube = np.concatenate(
                            (final_cube, cubes[c][f][np.newaxis, ...] -
                             psf.clip(0)), axis=0)
                ...
                if args.stat == True:
                    print("\n STAT: Frame processing took: " +
                          str(MPI.Wtime() - t0_trans) + " s = " +
                          graphic_nompi_lib.humanize_time(MPI.Wtime() -
                                                          t0_frame))
                graphic_mpi_lib.dprint(
                        d == -31 and not new_info is None,
                        str(f) + " new_info " + str(len(new_info)))
                graphic_mpi_lib.dprint(
                        d == -31 and not final_cube is None,
                        "final_cube.shape: " + str(final_cube.shape))
                graphic_mpi_lib.dprint(d == -31 and final_cube is None,
                                       "final_cube: " + str(final_cube))

        if empty_frame == hdr[
                'NAXIS3']:  # cubes[c].shape[0]-1:  #All frames are empty.
            final_cube = None

        if final_cube is None:
            graphic_nompi_lib.iprint(
                    interactive, "\n No cube generated for " +
                    str(cube_list['cube_filename'][c]) +
                    "! You should consider relaxing n_fwhm, rmin, or tmax conditions."
            )
            ...
            if 'CRPIX1' in hdr.keys() and 'CRPIX2' in hdr.keys():
                hdr["CRPIX1"] = ('{0:14.7G}'.format(
                        final_cube.shape[1] / 2. + float(hdr['CRPIX1']) -
                        cube_list['info'][c][hdr['NAXIS3'] // 2, 4]), "")
                hdr["CRPIX2"] = ('{0:14.7G}'.format(
                        final_cube.shape[2] / 2. + float(hdr['CRPIX2']) -
                        cube_list['info'][c][hdr['NAXIS3'] // 2, 5]), "")

            hdr['history'] = "Updated CRPIX1, CRPIX2"
            ## graphic_mpi_lib.save_fits(psf_sub_filename, final_cube, hdr=hdr, backend='pyfits')
            graphic_nompi_lib.save_fits(psf_sub_filename, final_cube, hdr=hdr,
                                        backend='pyfits')
            ...
            graphic_nompi_lib.iprint(
                    ...
                    .format(
                            name=psf_sub_filename,
                            human_time=graphic_nompi_lib.humanize_time(
                                    MPI.Wtime() - t0_cube),
                            rate=os.path.getsize(psf_sub_filename) /
                            (1048576 * (MPI.Wtime() - t0_cube)),
                            cubes_in_mem=len(cubes.keys())))
            graphic_nompi_lib.iprint(
                    interactive,
                    "Remaining time: " + graphic_nompi_lib.humanize_time(
                            (MPI.Wtime() - t_init) *
                            (len(cube_list['cube_filename']) - c) /
                            (c - skipped + 1)) + "\n")
    ...
    if nici:
        print('--pattern ' + psf_sub_filename.split('_S')[0] +
              ' --info_pattern ' + info_filename.split('_S')[0])
    elif sphere:
        print('--pattern ' + psf_sub_filename.split('SPHER')[0] +
              ' --info_pattern ' + info_filename.split('SPHER')[0])
    elif scexao:
        print('--pattern ' + psf_sub_filename.split('HIC')[0] +
              ' --info_pattern ' + info_filename.split('HIC')[0])
    else:
        print('--pattern ' + psf_sub_filename.split('NACO')[0] +
              ' --info_pattern ' + info_filename.split('NACO')[0])
    sys.exit(0)
...
#######################################################################
    ...
    while not todo == "over":
        if todo == "median":
            ...
            if d > 2:
                print(
                        str(rank) + " received info_stack with shape " +
                        str(info_stack.shape) + ": " + str(info_stack))
            ...
            if d > 2:
                if stack is None:
                    ...
                    print(
                            str(rank) + " received stack, shape=" +
                            str(stack.shape) + ' info_stack.shape=' +
                            str(info_stack.shape))

            if not (stack is None or len(stack.shape) < 3):
                if 2 * l_max < stack.shape[1]:
                    bigstack = np.zeros((stack.shape[0], stack.shape[1] * 2,
                                         stack.shape[2] * 2))
                    smallstack = np.zeros(
                            (stack.shape[0], l_max * 2, l_max * 2))
                    bigstack[:, stack.shape[1] // 2:3 * stack.shape[1] // 2,
                             stack.shape[2] // 2:3 * stack.shape[2] //
                             2] = stack
                else:
                    bigstack = np.zeros((stack.shape[0], l_max * 2, l_max * 2))
                    bigstack[:, l_max - stack.shape[1] // 2:l_max +
                             stack.shape[1] // 2,
                             l_max - stack.shape[2] // 2:l_max +
                             stack.shape[2] // 2] = stack
                    ## print(bigstack[:,
                ...
                for fn in range(bigstack.shape[0]):
                    graphic_mpi_lib.dprint(
                            d > 2, "recentreing frame: " + str(fn) +
                            " with shape: " + str(bigstack[fn].shape) +
                            ' chunk started at: ' + str(s))
                    if info_stack[s + fn, 4] == -1 or info_stack[
                            s + fn, 5] == -1 or info_stack[s + fn, 6] == -1:
                        bigstack[fn] = np.NaN
                        continue
                    # Shift is given by (image centre position)-(star position)
                    if nofft == True:  # Use interpolation
                        if 2 * l_max < stack_shape[1]:
                            smallstack[fn] = ndimage.interpolation.shift(
                                    bigstack[fn], (stack_shape[1] / 2. -
                                                   info_stack[s + fn, 4],
                                                   stack_shape[2] / 2. -
                                                   info_stack[s + fn, 5]),
                                    order=3, mode='constant', cval=np.NaN,
                                    prefilter=False)[bigstack.shape[1] // 2 -
                                                     l_max:l_max +
                                                     bigstack.shape[1] // 2,
                                                     bigstack.shape[2] // 2 -
                                                     l_max:l_max +
                            ...
                            bigstack[fn] = ndimage.interpolation.shift(
                                    bigstack[fn], (stack_shape[1] / 2. -
                                                   info_stack[s + fn, 4],
                                                   stack_shape[2] / 2. -
                                                   info_stack[s + fn, 5]),
                        ...
                        if 2 * l_max < stack_shape[1]:
                            smallstack[fn] = graphic_nompi_lib.fft_shift(
                                    bigstack[fn],
                                    stack_shape[1] / 2. - info_stack[s + fn, 4],
                                    stack_shape[2] / 2. - info_stack[s + fn, 5]
                            )[bigstack.shape[1] // 2 - l_max:l_max +
                              bigstack.shape[1] // 2, bigstack.shape[2] // 2 -
                              l_max:l_max + bigstack.shape[2] // 2]
                        else:
                            bigstack[fn] = graphic_nompi_lib.fft_shift(
                                    bigstack[fn],
                                    stack_shape[1] / 2. - info_stack[s + fn, 4],
                                    stack_shape[2] / 2. - info_stack[s + fn, 5])
                    ...
                    if 2 * l_max > stack_shape[1]:
                        bigstack[fn, :int(
                                np.ceil(l_max - info_stack[s + fn, 4] -
                                        0.5)), :] = np.NaN
                        bigstack[fn,
                                 int(
                                         np.floor(l_max -
                                                  info_stack[s + fn, 4] + 0.5 +
                                                  stack_shape[1])):, :] = np.NaN
                        bigstack[fn, :, :int(
                                np.ceil(l_max - info_stack[s + fn, 5] -
                                        0.5))] = np.NaN
                        bigstack[fn, :,
                                 int(
                                         np.floor(l_max -
                                                  info_stack[s + fn, 5] + 0.5 +
                                                  stack_shape[2])):] = np.NaN
                if 2 * l_max < stack_shape[1]:
                    bigstack = smallstack.copy()
                    del smallstack
                graphic_mpi_lib.dprint(
                        d > 2,
                        "Sending back bigstack, shape=" + str(bigstack.shape))
