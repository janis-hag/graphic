	...
	for c in range(len(cube_list['cube_filename'])): # Loop over the cubes
		...
		for f in range(len(cube_list['info'][c])): # Loop over the frames in the cube
			...
			if stack is None or sum(len(i) for i in valid.itervalues())==0:
				...
				graphic_nompi_lib.iprint(interactive, "\r\r\r Processing cube ["+str(c+1)+"/"+str(len(cube_list['cube_filename']))+"]: "+str(cube_list['cube_filename'][c])+", frame "+str(f+1)+"/"+str(len(cube_list['info'][c]))+" calculating median. Kept "+str(si)+" out of "+str(sum(len(i) for i in valid.itervalues()))+" valid frames.")
