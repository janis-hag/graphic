...
def noise_vs_radius(image, n_radii, r_min, fwhm, r_max='Default', mad=True,
    ...
    detec_r = np.linspace(r_min, r_max,
                          n_radii)  # the mean radii of each annulus
    ...
    for r in np.arange(n_radii):
        ...
        for rep in range(reps):
            ...
            for circle_ix in range(n_circles):
                ...
                circle_x = detec_r[r] * np.sin(circle_ix * 2 * np.pi /
                                               n_circles + azimuth_offset)
                circle_y = detec_r[r] * np.cos(circle_ix * 2 * np.pi /
                                               n_circles + azimuth_offset)

                # What is the distance from the circle centre?
                circ_dist_map = np.sqrt((xx_annulus - circle_x)**2 +
                                        (yy_annulus - circle_y)**2)
                ...
                noise_circles[circle_ix] = np.sum(image_annulus * fraction /
                                                  np.nansum(fraction))
            ...
            if mad:
                noise_reps[rep] = 1.4862 * np.nanmedian(
                        np.abs(noise_circles - np.nanmedian(noise_circles)))
            elif robust_sigma:
...
def mean_vs_radius(image, n_radii, r_min, fwhm, median=False):
    ...
    # Get the arrays ready
    detec_r = np.linspace(r_min, npix / 2 - fwhm / 2.,
                          n_radii)  # the mean radii of each annulus
...
def prepare_detection_image(filename, save_name=None, smooth_image_length=None,
                            median_filter_length=None,
                            convolve_with_circular_aperture=None):
    '''
    ...
    if isinstance(filename, str):
        ...
        image = np.array(
                image,
                dtype=np.float)  # To avoid endian problems with bottleneck
    else:
    ...
    if median_filter_length:
        ...
        for x in range(image.shape[0]):
            for y in range(image.shape[1]):
                ...
                xmx = np.min([
                        image.shape[0] - 1, x + median_filter_length // 2
                ])
                ymn = np.max([0, y - median_filter_length // 2])
                ymx = np.min([
                        image.shape[1] - 1, y + median_filter_length // 2
                ])
                smoothed_image[x, y] = bottleneck.nanmedian(image[xmn:xmx,
                                                                  ymn:ymx])
...
def contrast_curve(filename, flux_filename, n_radii=200, r_min=5., r_max=None,
                   fwhm=4.5, remove_planet=False, planet_position=None,
                   planet_radius=10., plate_scale=0.027, n_sigma=5, offset=0,
                   label='', plot=False, return_noise=False,
                   sss_correction=True, self_subtraction_file=None,
    ...
    if remove_planet:
        ...
        image[planet_position[0] - planet_radius:planet_position[0] +
              planet_radius, planet_position[1] -
              planet_radius:planet_position[1] + planet_radius] = np.nan

    print('  Calculating contrast')
    # Measure the noise as a function of radius
    if use_apertures:
        noise, detec_r = noise_vs_radius_apertures(image, n_radii, r_min, fwhm,
                                                   r_max, reps=20, mad=mad,
                                                   robust_sigma=robust_sigma)
    else:
        noise, detec_r = noise_vs_radius(image, n_radii, r_min, fwhm,
                                         r_max=r_max, mad=mad,
                                         robust_sigma=robust_sigma)
    ...
    if sss_correction:
        n_res_elements = np.floor(2 * np.pi * detec_r /
                                  fwhm)  #number of resolution elements
        sss_corr_factor = np.sqrt(1 + 1. / (n_res_elements - 1))
        cl = stats.norm.cdf(
                n_sigma)  # confidence limit for the sigma value that was given
    ...
    if self_subtraction_file:
        ...
        correction_factor_interp = interpolate.interp1d(
                correction_seps, correction_factors, kind='linear',
    ...
    if save_contrast:
        con_head = 'Separation (arcsec) Contrast (mag)  using plate scale of ' + str(
                plate_scale)
    ...
    if save_noise:
        np.savetxt(save_noise, [detec_r, noise],
                   header='Separation (pix) NoiseSigma (counts)')
...
def snr_map_quick(image_file, fwhm_pix, n_radii, r_min=3., plot=False,
                  ...
                  save_name='snr_map.fits', robust_sigma=False, mad=False):
    ...
    noise_vs_rad, detec_r = noise_vs_radius(noise_image, n_radii, r_min,
                                            fwhm_pix, mad=mad,
                                            robust_sigma=robust_sigma)
    ...
    if save_name:
        pyfits.writeto(save_name, snr_map, header=header, overwrite=True,
...
def snr_map(image_file, noise_file, plot=False, remove_planet=False,
            planet_position=None, planet_radius=10., save_name='snr_map.fits'):
...
def inject_companions(cube_in, psf, parangs_rad, radii, fluxes,
    ...
    pad_psf[pad_psf.shape[0] // 2 - psf.shape[0] // 2:pad_psf.shape[0] // 2 +
            psf.shape[0] // 2,
            pad_psf.shape[1] // 2 - psf.shape[1] // 2:pad_psf.shape[1] // 2 +
            psf.shape[1] // 2] = 1 * psf
    ...
    cube2[:, cube_extra_pad:-cube_extra_pad,
          cube_extra_pad:-cube_extra_pad] = cube
    ...
    for frame_ix, frame in enumerate(cube2):
        ...
        for sep_ix, sep in enumerate(radii):
            ...
            elif ((x_ix_min + pad_psf.shape[0]) > frame.shape[0]) or (
                    (y_ix_min + pad_psf.shape[1]) > frame.shape[1]):
                print('Injected psf frame too close to edge!')
...
def fit_injected_companions(adi_image, psf, radii, fluxes, azimuth_offset=0.,
    ...
    for sep_ix, sep in enumerate(radii):
        ...
        psf_region = flux * shifted_psf[
                psf_shape[0] // 2 - cutout_radius:psf_shape[0] // 2 +
                cutout_radius, psf_shape[1] // 2 -
                cutout_radius:psf_shape[1] // 2 + cutout_radius]
...
# TODO find a better solution than using the absolute value
        measured_throughputs[sep_ix] = np.abs(
                scipy.optimize.fmin(flux_func, [1], xtol=1e-3, ftol=1e-3,
                                    args=(region, psf_region), disp=False))
        #measured_throughputs[sep_ix]=scipy.optimize.fmin(flux_func,[1],
        #                xtol=1e-3,ftol=1e-3,args=(region,psf_region),disp=False)
...
def naco_nd(nd_filter):
    ...
    if nd_filter == 'ND_Long':
        ...
        print('  WARNING: Unknown ND filter ' + str(nd_filter) +
              ' in graphic_contrast_lib!')
        return 1.
