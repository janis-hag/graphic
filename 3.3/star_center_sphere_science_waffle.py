...
if rank==0:
    ...
    def star_center(key_word,science_waffle=False):
        """
        ...
        if science_waffle:
            ...
            for allfiles in glob.iglob(key_word):
                ...
                count+=1
            if len(np.shape(im_waffle))>2: #if it is a cube we take the mean
                ...
                im_waffle=np.median(im_waffle,axis=0) #taking the mean
            size_cube=1
        
        for i in range(size_cube):
            if science_waffle:
                im_waffle=cube_waffle[i,:,:]
            ...
            x2=np.arange(-l/2,l/2)
            y2=x2
            X2,Y2=np.meshgrid(x2,y2)
            R2=np.sqrt(X2**2+Y2**2)

            model=twoD_Gaussian((X2,Y2), 1, 0, 0, 3, 3,0)
            mask=np.where(R2>4,-1,1)
            mask=np.where(R2>6,0,mask)

            model=model*mask
            low_pass_im1=signal.correlate2d(im1_donut,model,"same")
            ...
            low_pass_im2=signal.correlate2d(im2_donut,model,"same")
            max_index_vec2=[]
            for i in range(4):
            ...
            for i in range(4):
                ...
                im1_temp=im1_donut[int(Prim_y1)-10:int(Prim_y1)+10,int(Prim_x1)-10:int(Prim_x1)+10]
                Prim_x1_temp=Prim_x1-int(Prim_x1)+10
                Prim_y1_temp=Prim_y1-int(Prim_y1)+10
                
                paramsinitial1=[np.max(im1_donut),5,5,Prim_x1_temp,Prim_y1_temp]
                fitobj1 = kmpfit.Fitter(residuals=error, data=(im1_temp,med1))
                fitobj1.fit(params0=paramsinitial1)
                par_vec1_temp=np.copy(fitobj1.params)
                par_vec1_temp[3]=par_vec1_temp[3]+int(Prim_x1)-10
                par_vec1_temp[4]=par_vec1_temp[4]+int(Prim_y1)-10
                par_vec1=np.append(par_vec1,par_vec1_temp)
            ...
            for i in range(4):
                ...
                #cutting the image just around the waffle to make the fit faster
                im2_temp=im2_donut[int(Prim_y2)-10:int(Prim_y2)+10,int(Prim_x2)-10:int(Prim_x2)+10]
                Prim_x2_temp=Prim_x2-int(Prim_x2)+10
                Prim_y2_temp=Prim_y2-int(Prim_y2)+10
                paramsinitial2=[np.max(im2_donut),5,5,Prim_x2_temp,Prim_y2_temp]
                fitobj2 = kmpfit.Fitter(residuals=error, data=(im2_temp,med2))
                fitobj2.fit(params0=paramsinitial2)
                par_vec2_temp=np.copy(fitobj2.params)
                par_vec2_temp[3]=par_vec2_temp[3]+int(Prim_x2)-10
                par_vec2_temp[4]=par_vec2_temp[4]+int(Prim_y2)-10
                par_vec2=np.append(par_vec2,par_vec2_temp)
    ...
    if science_waffle:
        ...
        for allfiles in glob.iglob(pattern+"*"):
            ...
            f.write(allfiles+' \n')
            f.close()
            star_center(allfiles,science_waffle)
    else:
        star_center(pattern+'*')
