...
def bcast_window_coord(x0=None, y0=None, xs=None, xe=None, ys=None, ye=None,
                       ref_window=None):
    '''
...
def cluster_search(image, thres_coef, min, max, x_i, y_i, d=0):
    ...
    if ima[x_i, y_i] > thres:  #check if above thresh
        ...
        # compute total luminosity of the spot
            ...
            tot_lum = numpy.sum(
                    numpy.abs(ima[new_cluster[i, 0], new_cluster[i, 1]])
                    for i in range(new_cluster.shape[0]))
            # find the xcentre of mass
            xcentre = numpy.sum(
                    new_cluster[i, 0] *
                    numpy.abs(ima[new_cluster[i, 0], new_cluster[i, 1]])
                    for i in range(new_cluster.shape[0])) / tot_lum
            ...
            if spot_ary is None:
                spot_ary = numpy.array([xcentre, ycentre,
                                        new_cluster.size])  # initialise
                ...
                dprint(
                        d > 0, 'Found bigger spot, old: ' + str(spot_ary) +
                        ', new: ' + str([xcentre, ycentre, new_cluster.size]))
                spot_ary = numpy.array([xcentre, ycentre, new_cluster.size])
            if check_ima[int(numpy.round(xcentre)),
                         int(numpy.round(ycentre))] == 0:
                check_ima[int(numpy.round(xcentre)),
                          int(numpy.round(ycentre)
                              )] = tot_lum  #Add a point on the checking image
            else:
                check_ima[int(numpy.round(xcentre)) + 1,
                          int(numpy.round(ycentre)) + 1] = tot_lum
        ...
        for ypix_cnt in range(ima.shape[1]):
            for xpix_cnt in range(ima.shape[0]):
                if ima[xpix_cnt, ypix_cnt] > thres:  #check if above thresh
                    new_cluster = numpy.array(
                            [])  # create a new empty cluster array
                    new_cluster = cluster_build(xpix_cnt, ypix_cnt, ima,
                                                new_cluster, thres)
                    #remove cluster from img to prevent double detection, by
                    #check the size of the spot
                    if new_cluster.size < min:
                    ...
                    #check if it is the first spot
                    if spot_ary is None:
                        spot_ary = numpy.array([
                                xcentre, ycentre, new_cluster.size
                        ])  # initialise
                    elif new_cluster.size > spot_ary[2]:
                        # More than one centroid detected. Keep only the biggest.
                        #spot_ary=numpy.vstack((spot_ary,[xcentre,ycentre,new_cluster.size]))
                        dprint(
                                d > 0, 'Found bigger spot, old: ' +
                                str(spot_ary) + ', new: ' +
                                str([xcentre, ycentre, new_cluster.size]))
                        spot_ary = numpy.array([
                                xcentre, ycentre, new_cluster.size
                        ])
                    if check_ima[int(numpy.round(xcentre)),
                ...
                continue
    if cl_cnt == 0:
        print(" No centroids found. Clusters closest to size limits: " +
              str(smallest) + " < [ " + str(min) + " : " + str(max) + " ] < " +
              str(biggest) + " \n")
        ## if not smallest==0:
        ...
        print(" Multiple centroids found. Clusters closest to size limits: " +
              str(smallest) + " < [ " + str(min) + " : " + str(max) + " ] < " +
              str(biggest) + " \n")
...
def cluster_search_multi(image, thres_coef, min, max, x_i, y_i):
    ...
    if ima[x_i, y_i] > thres:  #check if above thresh
        ...
        # compute total luminosity of the spot
            ...
            if spot_ary is None:
                ...
                spot_ary = numpy.vstack(
                        (spot_ary, [xcentre, ycentre, new_cluster.size]))
            if check_ima[int(numpy.round(xcentre)),
                         int(numpy.round(ycentre))] == 0:
                check_ima[int(numpy.round(xcentre)),
                          int(numpy.round(ycentre)
    ...
    if cl_cnt == 0:
        sys.stdout.write(
                " No centroids found. Clusters closest to size limits: " +
                str(smallest) + " < [ " + str(min) + " : " + str(max) +
                " ] < " + str(biggest) + " \n")
        ## if not smallest==0:
        ##     sys.stdout.write("Biggest candidate below limit size: "+str(smallest)+".\n")
        ...
        sys.stdout.write(
                " Multiple centroids found. Clusters closest to size limits: " +
                str(smallest) + " < [ " + str(min) + " : " + str(max) +
                " ] < " + str(biggest) + " \n")
...
def cluster_build(xpos, ypos, image, cl_ary, thr):
    ...
    if image[xpos, ypos] > thr:
        ...
        if cl_ary.size > 0:  # see if cl_ary has already been filled
            ...
            cl_ary = numpy.array([xpos, ypos])  #initiate the array
        #print cl_ary.shape
...
def send_chunks(cub_in, d):
    ...
    if cub_in.shape[1] / 2. < (
            nprocs - 1):  # Send only two or three columns to each proc
        for n in range(nprocs - 1):
            if 2 * n + 2 > cub_in.shape[1]:
                comm.send(cub_in.shape[1], dest=n + 1)
                comm.send(None, dest=n + 1)
            elif 2 * n + 3 == cub_in.shape[
                    1]:  # Reaching the end of the cube, send all the remaining frames.
        ...
        for n in range(nprocs - 1):
            start = int(n * numpy.floor(float(cub_in.shape[1]) / (nprocs - 1)))
                      ...
                      numpy.floor(float(cub_in.shape[1]) / (nprocs - 1)))
            if n + 1 == nprocs - 1:
                end = cub_in.shape[1]
            ...
            if d > 3:
                print("Chunk " + str(n) + " sent, shape: " +
                      str(cub_in[:, start:end, :].shape))
    del cub_in
...
def send_frames(cub_in):
    ...
    if cub_in.shape[0] / 2. < (nprocs -
                               1):  # Send only two or three frames to each proc
        for n in range(nprocs - 1):
            if 2 * n + 2 > cub_in.shape[0]:
                comm.send(cub_in.shape[0], dest=n + 1)
                comm.send(None, dest=n + 1)
            elif 2 * n + 3 == cub_in.shape[
                    0]:  # Reaching the end of the cube, send all the remaining frames.
                comm.send(2 * n, dest=n + 1)
                comm.send(cub_in[2 * n:, :, :], dest=n + 1)
            else:
                comm.send(2 * n, dest=n + 1)
        ...
        for n in range(nprocs - 1):
            start = int(n * numpy.floor(float(cub_in.shape[0]) / (nprocs - 1)))
                      ...
                      numpy.floor(float(cub_in.shape[0]) / (nprocs - 1)))
            #if end >= cub_in.shape[0]:
            if n + 1 == nprocs - 1:
                end = cub_in.shape[0]
            comm.send(start, dest=n + 1)
