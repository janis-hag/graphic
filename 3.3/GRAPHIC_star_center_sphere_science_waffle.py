...
parser.add_argument(
        ...
        const=True, default=False, help='Switch to science frame with waffle' +
...
parser.add_argument(
        '--lowpass_r', dest='lowpass_r', action='store', type=int, default=50,
        help='Radius of low pass filter to apply' +
        ' (in pixels) prior to the rough guess of the centre.' + ' Default 50.')
parser.add_argument(
        '-default_rough_centre', dest='default_centre', action='store_const',
...
parser.add_argument(
        '--manual_rough_centre', dest='manual_rough_centre', action='store',
        type=int, nargs='+', default=-1,
        help='Rough position of star behind coronagraph. Use this' +
        ' to overwrite the rough centring before the waffle' +
        ' positions are measured.')
parser.add_argument(
        '--ignore_frame', dest='ignore_frame', type=float, default=None,
        nargs='+', required=False,
        help='ds9 number of frames for' + ' bad frames to be ignored')
parser.add_argument(
        '-gaussian_fit', dest='gaussian_fit', action='store_const', const=True,
        default=False,
...
if rank == 0:
    ...
    def star_centre(key_word, science_waffle=False, ifs=False, lowpass_r=50,
                    manual_rough_centre=-1):
        """
        ...
        if ifs:
            ...
            if science_waffle:
                cube_waffle = np.array([
                        cube_waffle[:, :, :1024], cube_waffle[:, :, 1024:]
                ])
...
#                               centre[0] - window//2:centre[0] + window//2]
                ...
                if ifs:
                    ...
                    # the re-cubing
                    diff_limit = (hdr['HIERARCH GC WAVELENGTH'] * 1e-6 /
                                  8.2) * 180 / np.pi * 3600 * 1000
                ...
                for i in range(4):
                    ...
                    max_index = np.array(
                            np.where(low_pass_im == np.nanmax(low_pass_im)))
                ...
                for i in range(4):
                    ...
                    centre_spot[i] = (
                            [max_index_vec1[2 * i], max_index_vec1[2 * i + 1]])
                    ...
                    im_temp = im_donut[int(centre_spot[i][0]) -
                                       cutout_sz:int(centre_spot[i][0]) +
                                       cutout_sz,
                                       int(centre_spot[i][1]) -
                                       cutout_sz:int(centre_spot[i][1]) +
                                       cutout_sz]
                    ...
                    if (((centre_spot[i][0] < centre[0]) &
                         (centre_spot[i][1] < centre[1])) or
                        ((centre_spot[i][0] > centre[0]) &
                    ...
                    if gaussian_fit:
                        ...
                        fit = gaussfit.psf_gaussfit(im_temp, width=3.5,
                                                    saturated=False)
                        fit_params = fit.parameters  # (amplitude, x0, y0, sigmax, sigmay, theta)
                        # Convert to pixels in original image
                        centre_fit = [
                                fit_params[2] - im_temp.shape[0] / 2. +
                                centre_spot[i][0] - im_donut.shape[0] / 2. +
                                centre[0], fit_params[1] -
                                im_temp.shape[1] / 2. + centre_spot[i][1] -
                                im_donut.shape[1] / 2. + centre[1]
                        ]

                        centre_spot[i] = centre_fit
                        correction = [
                                fit_params[2] - im_temp.shape[0] / 2.,
                                fit_params[1] - im_temp.shape[1] / 2.
                        ]
                        ...
                        paramsinitial = [
                                np.nanmedian(im_temp),
                                np.nanmax(im_temp), 0.1, 0.1, 7, 9, 7,
                                theta_init
                        ]
                        ...
                        par_vec_temp[1] = par_vec_temp[
                                1] + cutout_sz  #+int(Prim_x)#-10
                        par_vec_temp[2] = par_vec_temp[
                                2] + cutout_sz  #+int(Prim_y)#-10
                        par_vec = np.append(par_vec, par_vec_temp)
                        par_init = np.append(par_init, paramsinitial)

                        centre_spot[i] = ([
                                centre[0] + centre_spot[i][0] +
                                par_vec_temp[1] -
                                np.shape(low_pass_im)[0] / 2. -
                                np.shape(im_temp)[0] / 2., centre[1] +
                                centre_spot[i][1] + par_vec_temp[2] -
                                np.shape(low_pass_im)[1] / 2. -
                                np.shape(im_temp)[1] / 2.
                        ])

                        correction = np.array([
                                par_vec_temp[1] - im_temp.shape[0] / 2.,
