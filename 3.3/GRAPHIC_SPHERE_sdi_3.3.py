...
parser.add_argument(
        '-additional', action="store_const", dest="additional", const=True,
        default=False, help='if True produce in \
...
parser.add_argument(
        '--r_int', action="store", dest="r_int", default=30, type=int,
        help='Interior radius (in pixels) for the area \
                    used to calculate the flux ratio between the cubes')
parser.add_argument(
        '--r_ext', action="store", dest="r_ext", default=80, type=int,
        help='Exterior radius (in pixels) for the area \
                    used to calculate the flux ratio between the cubes')
...
for allfiles in glob.iglob(pattern + '*'):
    ...
    rescaled_cube = graphic_nompi_lib.rescale_image(
            scaled_cube, reference_lambda / scaled_lambda,
            reference_lambda / scaled_lambda)

    # Work out the flux scaling factor needed to best subtract the PSF
    flux_factor = graphic_nompi_lib.scale_flux(reference_cube, rescaled_cube,
    ...
    sys.stdout.write('\n')
    sys.stdout.write('  Scaling flux of ' + rescaled_channel_name +
                     ' cube by ' + str(flux_factor))
    sys.stdout.flush()

    # Write it out
    #    pyfits.writeto(allfiles.replace('left','sdi'),sdi_cube,header=hdr,clobber=True)
    graphic_nompi_lib.save_fits(allfiles.replace('left', 'sdi'), sdi_cube,
                                hdr=hdr, backend='pyfits')
    if additional:
        scaled_cube_output_name = allfiles.replace(
                'left', rescaled_channel_name + '_rescale')
