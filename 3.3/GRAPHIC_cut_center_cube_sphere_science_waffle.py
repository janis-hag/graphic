...
parser = argparse.ArgumentParser(description=
                                 'Cut and center (with the file created by the'
...
if rank == 0:
    def translat_cube(cube, x, y):
        ...
        plan_fft = pyfftw.n_byte_align_empty(
        ...
        fft = pyfftw.n_byte_align_empty(
                (np.shape(cube)[1], np.shape(cube)[2]), 16, 'complex128')
        fft_translate = pyfftw.n_byte_align_empty(
                (np.shape(cube)[1], np.shape(cube)[2]), 16, 'complex128')
        cube_translate = pyfftw.n_byte_align_empty(
                np.shape(cube), 16, 'complex128')
        for i in range(np.shape(cube)[0]):
            ...
            if np.size(x) > 1:
                ...
                #  individually
                fft_translate = np.transpose(np.transpose(
                        fft*np.exp(-2*np.pi*1j*(x[i]*k)))*np.exp(-2*np.pi*1j*(y[i]*k)))
            else:
                fft_translate = np.transpose(np.transpose(
                        fft*np.exp(-2*np.pi*1j*(x*k)))*np.exp(-2*np.pi*1j*(y*k)))
    ...
    def cut_irdis_cube(cube, hdr):
        ...
        mask_l = np.where(
                R >= r_l-d_ap/2.,
                (1.+np.cos(np.pi+(np.pi)*(r_l-d_ap/2.-R+d_ap)/d_ap))/2., 1)
        mask_l = np.where(R > r_l-d_ap/2. + d_ap, 0, mask_l)
        mask_r = np.where(
                R >= r_r-d_ap/2.,
                (1.+np.cos(np.pi+(np.pi)*(r_r-d_ap/2.-R+d_ap)/d_ap))/2., 1)
...
#                    fftpack.ifftshift(fft_r), threads=4)
    ...
    def clean_ifs_cube(cube, hdr, square_length=220, lenslet_rotation=-10.7,
                       cutoff_radius=120, cutoff_region=50.,
        ...
        mask_nan = np.repeat(mask_nan[np.newaxis, :, :],
                             repeats=cube.shape[0], axis=0)
        ...
        if lowpass_filter:
            ...
            mask = np.where(
                    R >= (cutoff_radius-cutoff_region/2.),
                    (1.+np.cos(np.pi+(np.pi)*(cutoff_radius-cutoff_region/2. - R + cutoff_region)/cutoff_region))/2., 1)
            mask = np.where(R > (cutoff_radius-cutoff_region/2.+cutoff_region),
                            0, mask)
            ...
            for i in range(np.shape(cube)[0]):
                ...
                cube[i, :, :] = np.real(pyfftw.interfaces.scipy_fftpack.ifft2(
                        fftpack.ifftshift(fft_l), threads=4))
    ...
    def cut_and_center_cub(file_name, science_waffle=False, ifs=False):
        ...
        for wav in range(n_wav):
            if science_waffle:
                ...
                nbr_pix1 = np.int(np.round(
                        (center[wav, 0, 1]-np.shape(cube[wav])[-2]/2.)))
                nbr_pix2 = np.int(np.round(
                        (center[wav, 0, 0]-np.shape(cube[wav])[-1]/2.)))
            else:
                # Calculate how many pixels to shift the NaN mask
                nbr_pix1 = np.int(np.round(
                        (center[wav, 1]-np.shape(cube[wav])[-2]/2.)))
                nbr_pix2 = np.int(np.round(
                        (center[wav, 0]-np.shape(cube[wav])[-1]/2.)))
        ...
        for wav in range(n_wav):
            ...
            if n_wav == 2:
                ...
                hdr['HIERARCH GC WAVELENGTH'] = (
                        filter_wav*1e-3, 'Wavelength in microns')
