...
parser = argparse.ArgumentParser(
        description='Cut and center (with the file created by the' +
        ' star center routine) the cubes for left' +
        ' and right images in SPHERE data')
parser.add_argument('--pattern', action="store", dest="pattern",
                    default="cl_nomed_SPHER*SCIENCE_DBI",
                    help='cubes to apply the cut and centering')
parser.add_argument(
        '-science_waffle', dest='science_waffle', action='store_const',
        const=True, default=False,
        help='Switch to science frame with waffle (usually for' +
...
parser.add_argument('--star_center_file', action="store",
                    dest="star_center_file", default='star_center.txt',
...
def translat_cube(cube, x, y):
    ...
    plan_fft = pyfftw.n_byte_align_empty((np.shape(cube)[1], np.shape(cube)[2]),
                                         16, 'complex128')
    plan_fft = pyfftw.interfaces.scipy_fftpack.fft2(
            plan_fft, planner_effort='FFTW_MEASURE', threads=4)
    fft = pyfftw.n_byte_align_empty((np.shape(cube)[1], np.shape(cube)[2]), 16,
                                    'complex128')
    fft_translate = pyfftw.n_byte_align_empty(
            (np.shape(cube)[1], np.shape(cube)[2]), 16, 'complex128')
    cube_translate = pyfftw.n_byte_align_empty(np.shape(cube), 16, 'complex128')
    for i in range(np.shape(cube)[0]):
        fft = pyfftw.interfaces.scipy_fftpack.fft2(
                cube[i, :, :], planner_effort='FFTW_MEASURE', threads=4)
        if np.size(x) > 1:
            #  in case of science_waffle we center each frames of the cube
            #  individually
            fft_translate = np.transpose(
                    np.transpose(fft * np.exp(-2 * np.pi * 1j * (x[i] * k))) *
                    np.exp(-2 * np.pi * 1j * (y[i] * k)))
        else:
            fft_translate = np.transpose(
                    np.transpose(fft * np.exp(-2 * np.pi * 1j * (x * k))) *
                    np.exp(-2 * np.pi * 1j * (y * k)))
        cube_translate[i, :, :] = pyfftw.interfaces.scipy_fftpack.ifft2(
                fft_translate, planner_effort='FFTW_MEASURE', threads=4)
...
def cut_irdis_cube(cube, hdr):
    ...
    known bad pixels at the same time
    '''
    ...
    mask_l = np.where(R >= r_l - d_ap / 2.,
                      (1. + np.cos(np.pi + (np.pi) *
                                   (r_l - d_ap / 2. - R + d_ap) / d_ap)) / 2.,
                      1)
    mask_l = np.where(R > r_l - d_ap / 2. + d_ap, 0, mask_l)
    mask_r = np.where(R >= r_r - d_ap / 2.,
                      (1. + np.cos(np.pi + (np.pi) *
                                   (r_r - d_ap / 2. - R + d_ap) / d_ap)) / 2.,
                      1)
...
def clean_ifs_cube(cube, hdr, square_length=220, lenslet_rotation=-10.7,
                   cutoff_radius=120, cutoff_region=50., lowpass_filter=False):
    '''
    Function to clean up the IFS cubes and make a NaN mask that shows
    which regions are outside of the field of view.
    The images are lowpass filtered using a cutoff_radius

    square_length is the size of the IFS field of view in pixels
    lenslet_rotation is the rotation of the square field of view with
    respect to the detector

    cutoff_radius is the radius (in pixels) used for the lowpass filter
    cutoff_region is the size of the region used to slowly ramp from 1. to
    0. in the filter
    ...
    mask_nan = np.nan + np.zeros((cube.shape[-2], cube.shape[-1]))
    # Just 1s for now
    ...
    mask_nan = np.repeat(mask_nan[np.newaxis, :, :], repeats=cube.shape[0],
                         axis=0)
    ...
    if lowpass_filter:
        ...
        mask = np.where(R >= (cutoff_radius - cutoff_region / 2.),
                        (1. + np.cos(np.pi + (np.pi) *
                                     (cutoff_radius - cutoff_region / 2. - R +
                                      cutoff_region) / cutoff_region)) / 2., 1)
        mask = np.where(
                R > (cutoff_radius - cutoff_region / 2. + cutoff_region), 0,
                mask)
        ...
        pyfftw.interfaces.cache.set_keepalive_time(30)
        for i in range(np.shape(cube)[0]):
            fft_l = fftpack.fftshift(
                    pyfftw.interfaces.scipy_fftpack.fft2(
                            cube[i, :, :], threads=4)) * mask
            cube[i, :, :] = np.real(
...
def cut_and_center_cub(file_name, science_waffle=False, ifs=False):
    ...
    produced by star_center_sphere.py

    Input: file name of the cube

    Output: produce 2 files, *_left.fits and *_right.fits with the cubes
    centered
    """
    # Read the cube
    cube, hdr = pyfits.getdata(file_name, header=True)
    """#applying a mask with median value on the dead pixels zone (
    # 1 on each filter)
    ...
    if not ifs:
        ...
        # Make sure the cube is 4D and get the number of wavelength
        # channels
        if cube.ndim == 3:
    ...
    if science_waffle:
        ...
        if ifs:
            # To fix it, make a loop over wavelength instead of assuming
            # left and right channels. However this requires knowing the
        ...
        for i, line in enumerate(lines):
            if i + 1 == np.size(lines):
                # testing if we are at the end of the star_center file
                index_next_file = i + 1
                break
            elif next_cube == 1:
                # testing if we are at the end of a cube
        ...
        for i in range((index_next_file - index_file - 1) / 2):
            left_center_temp = np.array([
                    np.float(lines[index_file + 2 * i + 1].strip().split()[1]),
                    np.float(lines[index_file + 2 * i + 1].strip().split()[2])
            ])
            right_center_temp = np.array([
                    np.float(lines[index_file + 2 * i + 2].strip().split()[1]),
                    np.float(lines[index_file + 2 * i + 2].strip().split()[2])
            ])
        ...
        for wav in range(n_wav):
            center[wav, :] = [
                    np.float(lines[wav + 2].strip().split()[1]),
    ...
    for wav in range(n_wav):
        if science_waffle:
            ...
            nbr_pix1 = np.int(
                    np.round(
                            (center[wav, 0, 1] - np.shape(cube[wav])[-2] / 2.)))
            nbr_pix2 = np.int(
                    np.round(
                            (center[wav, 0, 0] - np.shape(cube[wav])[-1] / 2.)))
        else:
            # Calculate how many pixels to shift the NaN mask
            nbr_pix1 = np.int(
                    np.round((center[wav, 1] - np.shape(cube[wav])[-2] / 2.)))
            nbr_pix2 = np.int(
                    np.round((center[wav, 0] - np.shape(cube[wav])[-1] / 2.)))

        # Shift it
        mask_nan[wav] = np.roll(mask_nan[wav], -nbr_pix1, axis=-2)
        mask_nan[wav] = np.roll(mask_nan[wav], -nbr_pix2, axis=-1)

        # Then NaN out the extra rows and columns from outside the array
        # that shifted in
        if nbr_pix1 > 0:
    ...
    if not ifs:
        ...
        if n_wav == 1:
            # if there's only 1 wavelength channel, we don't need to
            # specify what wavelength it is
            prefixes = ['cen_']
        else:
            prefixes = ['wav' + str(wav) + '_' for wav in range(n_wav)]

    for wav in range(n_wav):
        # Add a header keyword saying which number channel this is
        # (left/right = 0/1). For IFS, this should already be added.
        ...
        if n_wav == 2:
            ...
            # #### J band
                ...
                raise Exception('ERROR! Unknown filter in \
                                cut_center_cube_sphere_science_waffle.py!')

            # The filter wavelenth file should be in a subdirectory from
            # the location of this file
            filter_wavelength_file = os.path.dirname(
                    os.path.realpath(__file__)
            ) + os.sep + 'SPHERE_characterization/photometry_SPHERE/filter_wavelength.dat'
            filters, filter_wavs = np.loadtxt(filter_wavelength_file,
                                              skiprows=2, unpack=True,
                                              dtype=str)
            filter_wavs = filter_wavs.astype(np.float64)
            filter_wav = filter_wavs[filters == filter_name][0]

            hdr['HIERARCH GC WAVELENGTH'] = (filter_wav * 1e-3,
                                             'Wavelength in microns')

        out_name = prefixes[wav] + file_name
        pyfits.writeto(out_name, cube_translat[wav], header=hdr,
                       output_verify='warn', overwrite=True)
