...
parser.add_argument('--pca_modes', dest='pca_modes', action='store', default=None, type=int,
...
if rank==0:
	...
	sky_array=np.reshape(sky_array,(orig_sky_shape[0],orig_sky_shape[1]*orig_sky_shape[2]))

	# Calculate the principal components
	sky_pcomps=pca.principal_components(sky_array,n_modes=pca_modes)

	print("  Max: "+str(np.max(sky_array))+" Min: "+str(np.min(sky_array)))
...
for i in range(len(dirlist)):
	...
	cube=np.reshape(cube,(orig_cube_shape[0],orig_cube_shape[1]*orig_cube_shape[2]))
	...
	if star_window ==0:
		...
		mask[orig_cube_shape[1]/2-star_window:orig_cube_shape[1]/2+star_window,
			 orig_cube_shape[2]/2-star_window:orig_cube_shape[2]/2+star_window] = False
		# Make it 1D
	...
	header['HIERARCH GC SKY_PCA_MODES']=( sky_pcomps.shape[0], '# of sky pca modes subtracted')

	graphic_nompi_lib.save_fits(targetfile, cube.astype(np.float32), hdr=header, backend='pyfits') # ACC removed verify='warn' because NACO files have a PXSPACE card that is non-standard
