...
import numpy, scipy, glob,  os, sys, string, time
import numpy as np
...
parser.add_argument('--lmax', action="store", dest="l_max", type=int, default=0,
...
parser.add_argument('--output_n_frames', dest='output_n_frames', action='store', default=0, type=int,
                    help="Combine frames in the input cubes to give this many frames per cube in the output. Default (0) takes all frames")
...
if rank == 0:
    ...
    for c in range(0, len(cube_list['cube_filename']), naxis3): # Loop over the output cubes
        t0_cube = MPI.Wtime()
        ...
        new_cube = None
        new_info = None
        for n in range(naxis3): # Loop over the files we need to bin together for this output cube
            ...
            cube = cube[:,int(cube.shape[1]/2-l_max_in/2):int(cube.shape[1]/2+l_max_in/2),
                                    int(cube.shape[2]/2-l_max_in/2):int(cube.shape[2]/2+l_max_in/2)]
            ...
            if collapse:
                ...
                new_info[n] = nanmedian(np.where(cube_list['info'][c+n] == -1, np.NaN, cube_list['info'][c+n]), axis=0)
                ...
                if output_n_frames != 0:
                    save_cube = np.zeros((output_n_frames,cube.shape[1],cube.shape[2]))
                    ...
                    hdr['HIERARCH GC INPUT_N_FRAMES'] = (input_cube_shape[0],"Number of original frames in cube")
                    hdr['HIERARCH GC OUTPUT_N_FRAMES'] = (output_n_frames,"Number of output frames requested")

                # Remove the bad frames that contain only NaNs
                bad_frames = (np.sum(np.isnan(cube) == False,axis=(1,2)) == 0)
                cube = cube[bad_frames == False]
                ...
                hdr['HIERARCH GC NAN_FRAMES_REMOVED'] = (np.sum(bad_frames),"NaN-only frames removed during recentring")

                # Convert it to 32bit float to save storage space (we dont need 64 bit)
                cube = cube.astype(np.float32)

                # Only save if there are some frames to save:
                n_good_frames = np.sum(bad_frames == False)
                if n_good_frames == 0:
        ...
        if collapse:
            ...
            if args.combine_frames:
                ...
                # Remove the bad frames that contain only NaNs
                bad_frames = (np.sum(np.isnan(new_cube) == False,axis=(1,2)) == 0)
                new_cube = new_cube[bad_frames == False]
                new_info = new_info[bad_frames == False]
                hdr['HIERARCH GC NAN_FRAMES_REMOVED'] = (np.sum(bad_frames),"NaN-only frames removed during recentring")
