...
parser.add_argument(
        '--lmax', action="store", dest="l_max", type=int, default=0,
        help='Shape of the final image. If not \
...
parser.add_argument('-nocollapse', dest='collapse', action='store_const',
                    const=False, default=True, help='Do not collapse cubes.')
parser.add_argument(
        '-combine_frames', dest='combine_frames', action='store_const',
...
parser.add_argument(
        ...
        default=False, help='Use bottleneck module \
...
parser.add_argument(
        '--output_n_frames', dest='output_n_frames', action='store', default=0,
        type=int, help="Combine frames \
...
if rank == 0:
    ...
    for c in range(0, len(cube_list['cube_filename']), naxis3):
        ...
        if nici:  # Convert JD to unix time to get rid of day change issues
            cube_list['info'][c][:, 10] = (cube_list['info'][c][:, 10] -
                                           2440587.5) * 86400
        ...
        for n in range(naxis3):
            if c + n == len(cube_list['cube_filename']):
                break
            print("Processing cube [" + str(c + n + 1) + "/" +
                  str(len(cube_list['cube_filename'])) + "]: " +
                  str(cube_list['cube_filename'][c + n]))
                ...
                for i in range(len(cube_list['info'])):
                    if not cube_list['info'][i][len(cube_list['info'][i]) // 2,
                        ...
                        length = 2 * graphic_nompi_lib.get_max_dim(
                                cube_list['info'][i][len(cube_list['info'][i]) //
                                                     2, 4], cube_list['info'][i]
                                [len(cube_list['info'][i]) // 2, 5],
                                int(hdr['NAXIS1']),
            ...
            if l_max < max_dim:
                ...
                xpos = [
                        temp_pos for this_cube_list in cube_list['info']
                        for temp_pos in this_cube_list[:, 4]
                ]
                ypos = [
                        temp_pos for this_cube_list in cube_list['info']
                        for temp_pos in this_cube_list[:, 5]
                ]
                if len(xpos) > 1:
            ...
            cube = cube[:,
                        int(cube.shape[1] // 2 -
                            l_max_in // 2):int(cube.shape[1] // 2 +
                                               l_max_in // 2),
                        int(cube.shape[2] // 2 -
                            l_max_in // 2):int(cube.shape[2] // 2 +
                                               l_max_in // 2)]
            ...
            if collapse:
                ...
                new_info[n] = nanmedian(
                        np.where(cube_list['info'][c + n] == -1, np.NaN,
                                 cube_list['info'][c + n]), axis=0)
                new_info[n] = np.where(np.isnan(new_info[n]), -1, new_info[n])
            else:
                psf_sub_filename = target_pattern + "_" + cube_list[
                        'cube_filename'][c + n]
                info_filename = "all_info_" + psf_sub_filename[:-5] + ".rdb"
                hdr['HIERARCH GC RECENTER'] = (str(__version__) + '.' +
                                               (__subversion__), "")
                ...
                hdr['CRPIX1'] = ('{0:14.7G}'.format(
                        cube.shape[1] / 2. + np.float(hdr['CRPIX1']) -
                        median(cube_list['info'][c + n][np.where(
                                cube_list['info'][c + n][:, 4] > 0), 4])), "")
                hdr['CRPIX2'] = ('{0:14.7G}'.format(
                        cube.shape[2] / 2. + np.float(hdr['CRPIX2']) -
                        median(cube_list['info'][c + n][np.where(
                                cube_list['info'][c + n][:, 5] > 0), 5])), "")

                hdr['history'] = 'Updated CRPIX1, CRPIX2'

                if output_n_frames != 0:
                    save_cube = np.zeros(
                            (output_n_frames, cube.shape[1], cube.shape[2]))
                    ...
                    for ix in range(output_n_frames):
                        ...
                        end_ix = start_ix + np.round(
                                float(frames_left) /
                                (output_n_frames - ix)).astype(int)
                        ...
                        frame_times.append(
                                nanmedian(cube_list['info'][c +
                                                            n][start_ix:end_ix,
                                                               10]))
                        paralactic_angles.append(
                                nanmedian(cube_list['info'][c +
                                                            n][start_ix:end_ix,
                                                               11]))
                        psf_fit_width_xs.append(
                                nanmedian(cube_list['info'][c +
                                                            n][start_ix:end_ix,
                                                               7]))
                        psf_fit_width_ys.append(
                                nanmedian(cube_list['info'][c +
                                                            n][start_ix:end_ix,
                                                               8]))
                    ...
                    new_info[:, 1] = np.repeat(
                            cube.shape[1] / 2, output_n_frames
                    )  # PSF Barycentre X (Since it has been recentred)
                    new_info[:,
                             2] = np.repeat(cube.shape[1] / 2,
                                            output_n_frames)  # PSF Barycentre Y
                    new_info[:,
                             4] = np.repeat(cube.shape[1] / 2,
                                            output_n_frames)  # PSF Fit centre X
                    new_info[:,
                             5] = np.repeat(cube.shape[1] / 2,
                                            output_n_frames)  # PSF Fit centre Y
                    new_info[:, 3] = np.repeat(
                            0, output_n_frames)  # PSF pixel size (?)
                    ...
                    hdr['HIERARCH GC INPUT_N_FRAMES'] = (
                            input_cube_shape[0],
                            "Number of original frames in cube")
                    hdr['HIERARCH GC OUTPUT_N_FRAMES'] = (
                            output_n_frames,
                ...
                hdr['HIERARCH GC NAN_FRAMES_REMOVED'] = (
                        np.sum(bad_frames),
        ...
        if collapse:
            ...
            if args.combine_frames:
                ...
                # Remove the bad frames that contain only NaNs
                bad_frames = (np.sum(np.isfinite(new_cube), axis=(1, 2)) == 0)
        ...
        print("Remaining time: " + graphic_nompi_lib.humanize_time(
    ...
    while not todo == "over":
        if todo == "median":
            ...
            if stack is not None:
                bigstack = np.zeros(
                        (stack.shape[0], l_max, l_max))  # ACC removed 2*
                ...
                bigstack[:, bigstack.shape[1] // 2 -
                         stack.shape[1] // 2:bigstack.shape[1] // 2 +
                         stack.shape[1] // 2 + extra_pix,
                         bigstack.shape[2] // 2 -
                         stack.shape[2] // 2:bigstack.shape[2] // 2 +
                         stack.shape[2] // 2 + extra_pix] = stack
                for fn in range(bigstack.shape[0]):
                    dprint(
                            d > 2, "recentreing frame: " + str(fn) +
                            " with shape: " + str(bigstack[fn].shape))

                    if info_stack[s + fn, 4] == -1 or info_stack[s + fn,
                                                                 5] == -1:
                        bigstack[fn] = np.NaN
                        continue
                    # Shift is given by (image centre position)-(star position)
                    if nofft == True:  # Use interpolation
                        shift_amount = (stack.shape[1] / 2. -
                                        info_stack[s + fn, 4],
                                        stack.shape[2] / 2. -
                                        info_stack[s + fn, 5])
                        bigstack[fn] = ndimage.interpolation.shift(
                                bigstack[fn], shift_amount, order=3,
                                mode='constant', cval=np.NaN, prefilter=False)
                    else:  # Shift in Fourier space
                        left_edge = int(
                                np.round(l_max // 2 - info_stack[s + fn, 4]))
                        right_edge = int(
                                np.round(l_max // 2 - info_stack[s + fn, 4] +
                                         stack.shape[1]))
                        bottom_edge = int(
                                np.round(l_max // 2 - info_stack[s + fn, 5]))
                        top_edge = int(
                                np.round(l_max // 2 - info_stack[s + fn, 5] +
                                         stack.shape[2]))
