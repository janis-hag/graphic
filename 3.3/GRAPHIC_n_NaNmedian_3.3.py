...
import glob, os, sys  #, fnmatch, string, time
import numpy as np
...
parser = argparse.ArgumentParser(
        description=
        'This program generates medians of a given number \"n\" of cubes. This can be used to generate flats.'
)
...
parser.add_argument('--win', action="store", dest="win", type=int, default=40,
                    help='Window length in pixels (40)')
parser.add_argument('--sky_dir', action="store", dest="target_dir",
                    default="sky-num", help='Info directory')
parser.add_argument('--log_file', action="store", dest="log_file",
                    default='GRAPHIC', help='Log filename')
parser.add_argument('--num', action="store", dest="num", default=-1, type=int,
                    help='Number offset positions to use')
...
parser.add_argument(
        '-mean', dest='mean', action='store_const', const=True, default=False,
...
parser.add_argument(
        '-quad', dest='noquad', action='store_const', const=False, default=True,
        help=
        'Ensure that all four quadrants are covered. Disbled by default as it is unreliable'
)
...
if rank == 0:
    ...
    for ix in range(len(cube_list['cube_filename'])):
        ...
        if len(this_skylist) == num:
            ...
            mean_obstimes[cube_list['cube_filename'][cube_ix]] = np.mean(
                    these_obstimes)
    ...
    for k in skylist.keys():
        ...
        header["HIERARCH GC SKY_OBSTIME"] = (mean_obstimes[k],
                                             "mean MJD of sky frame")

        i = 1

        for cube_name in skylist[k]:
            ci = cube_list['cube_filename'].index(cube_name)
            sys.stdout.write('\r\r\r Reading cube ' + str(i) + ' of ' +
                             str(len(skylist[k])) + ' : ' + str(cube_name))
            sys.stdout.flush()
            i = i + 1
            ...
            if nici:
                ...
                for f in range(len(cube_in.shape)):
                    cube_in[f] = graphic_nompi_lib.nanmask_frame_nici(
                            cube_list['info'][ci][f, 4],
                            cube_list['info'][ci][f, 5], cube_in[f], R, d)
            else:
                ...
                ## cube_in=graphic_nompi_lib.nanmask_cube(cube_list['info'][ci][int(header_in['NAXIS3'])/2,4], cube_list['info'][ci][int(header_in['NAXIS3'])/2,5], cube_in, R, d)
                    ...
                    cube_in = graphic_nompi_lib.nanmask_cube(
                            np.mean(cube_list['info'][ci][valid, 4]),
                            np.mean(cube_list['info'][ci][valid, 5]), cube_in,
                            R, d)
        ...
        for p in range(nprocs - 1):
            ...
            graphic_mpi_lib.dprint(
                    d > 0, "Received median reduced chunk from " + str(p + 1))
            if p == 0:  #initialise
                sky = r
            else:
        ...
        graphic_nompi_lib.save_fits(skyfile, sky, hdr=header,
                                    target_dir=target_dir, backend='pyfits')
        ...
        if isinstance(start, str) or isinstance(data_in, str):
            if start == "compute" or data_in == "compute":
                ...
                try:
                    data_in.shape
                    dprint(
                            d > 1, " received cube shape: " +
                            str(data_in.shape) + ", start: " + str(start))
                except:
                    dprint(d > 1,
                           " instead of cube chunk received: " + str(data_in))
            ...
            if type(data_in) == type(None) or type(my_cube) == type(None):
                dprint(
                        d > 1, "start: " + str(start) + ", my_cube: " +
                        str(my_cube) + ", data_in: " + str(data_in))
            else:
                dprint(
                        d > 1, "start: " + str(start) + ", my_cube.shape: " +
                        str(my_cube.shape) + ", data_in.shape: " +
                        str(data_in.shape))
