...
parser.add_argument('--psf_width', action="store", dest="psf_width", type=float,
                    default=4, help='An initial guess for the psf width')
parser.add_argument('--agpm_width', action="store", dest="agpm_width",
                    type=float, default=1.5,
                    help='An initial guess for the agpm width')
parser.add_argument('--agpm_offset_file', action="store",
                    dest="agpm_offset_file", type=str,
                    default='../Sky/agpm_offset.txt',
                    help='File containing measured AGPM offset')
...
parser.add_argument(
        '-fit_1d', dest='fit_1d', action='store_const', const=True,
        default=False,
...
if rank == 0:  # Master process
    ...
    print("Using AGPM offset of (x,y) = (" + str(mean_agpm_offset[0]) + "," +
          str(mean_agpm_offset[1]) + ")")
    ...
    for i in range(len(dirlist)):
        ...
        big_circle_cen, agpm_rad = gaussfit.fit_to_big_circle(
                mean_frame, fit_1d=fit_1d)
        ...
        if ('ESO DET NDIT'
                    in cube_header.keys()) and (cube_header['NAXIS3']
                                                != cube_header['ESO DET NDIT']):
            cent_list[-1] = -1

        # Set first frame to invalid for L_prime band due to cube reset effects
        if 'ESO INS OPTI6 ID' in cube_header.keys(
    ...
    graphic_nompi_lib.write_log_hdr((MPI.Wtime() - t_init), log_file,
                                    cube_header, comments, nprocs=nprocs)
...
## sys.exit(1)
    ...
    while not type(data_in) == type("over"):
        if not data_in is None and isinstance(data_in, np.ndarray):
            ...
            cut_agpm_pos = cut_agpm_pos[::
                                        -1]  # I think I got the axes wrong again...
            ...
            dist = np.sqrt((xx - (agpm_pos[1] - agpm_pos_pix[1]))**2 +
                           (yy - (agpm_pos[0] - agpm_pos_pix[0]))**2)
            ...
            for frame in range(data_in.shape[0]):
                ...
                im_cut = image[agpm_pos_pix[0] - cut_sz:agpm_pos_pix[0] +
                               cut_sz, agpm_pos_pix[1] -
                               cut_sz:agpm_pos_pix[1] + cut_sz]

                # Do the fitting
                fit = gaussfit.fixed_agpm_gaussfit(im_cut, width=psf_width,
                                                   agpm_width=agpm_width,
                                                   agpm_rejection_fraction=0.97,
                                                   agpm_position=cut_agpm_pos)
                ...
                centre_fit = star_params[
                        2:0:
                        -1] + agpm_pos_pix - cut_sz  # the output of agpm_gaussfit is relative to the edge of the cut frame
                ...
                cluster_array_ref = np.array([
                        frame + startframe, agpm_pos[0], agpm_pos[1], 0.,
                        centre_fit[0], centre_fit[1], donut_mean, donut_sigma,
                        donut_sigma
                ])
