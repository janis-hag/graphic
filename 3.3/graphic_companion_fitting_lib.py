...
#        return -np.Inf
    ...
    cube=graphic_contrast_lib.inject_companions(cube,psf,parangs_rad,radii,fluxes,
                   azimuth_offset=azimuth_offset,psf_pad=10,
                   silent=True,save_name=None)
...
#    final_im[nan_mask] = np.nan
    ...
    xpos = np.int(fit_params[0]*np.sin(-fit_params[1]*np.pi/180.)+final_im.shape[0]/2)
    ypos = np.int(fit_params[0]*np.cos(-fit_params[1]*np.pi/180.)+final_im.shape[0]/2)
...
def companion_mcmc(image,psf_image,initial_guess,image_err=1.,n_walkers=50.,n_iterations=1e3,
    ...
    if plot==True:
        ...
        for i in range(n_params):
            ...
            plt.hist(flatchain[:,i], np.int((n_iterations)**(0.5)),color=color, histtype="step")            
            ...
            for j in range(i):
                ...
                plt.hist2d(flatchain[:,i],flatchain[:,j],np.int(0.5*(n_iterations)**(0.5)))
...
def simple_companion_leastsq(image,psf_image,initial_guess,image_err=1.,threads=3,
    ...
    if fit_to_stdev:
        def min_funct(params,image,psf_image,image_err):
            stdev = simple_companion_stdev(params,image,psf_image,image_err=image_err)
            return stdev
    else:
        def min_funct(params,image,psf_image,image_err):
            loglike = simple_companion_loglikelihood(params,image,psf_image,image_err=image_err)
            return (-loglike)
    
    result = optimize.minimize(min_funct, initial_guess, args=(image,psf_image,image_err),tol=1e-4,method=method)

    return result

########################

def simple_companion_flux_leastsq(image,psf_image,initial_guess,image_err=1.,threads=3,plot=False,method='Powell'):
    ...
    result = optimize.minimize(flux_min_funct, guess,tol=1e-5,method=method,
           args=(initial_guess[0],initial_guess[1],image,psf_image,image_err))
