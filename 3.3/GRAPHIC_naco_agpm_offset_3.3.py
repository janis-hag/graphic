...
parser.add_argument('--psf_width', action="store", dest="psf_width", type=float,
                    default=2, help='An initial guess for the psf width')
...
for ix, filename in enumerate(proc_dirlist):
    ...
    rough_agpm_cen = np.array([
            big_circle_cen_pix[0] + rough_agpm_offset[0],
            big_circle_cen_pix[1] + rough_agpm_offset[1]
    ], dtype=int)
    im_cut = 1 * clean_sky[rough_agpm_cen[0] - cut_sz:rough_agpm_cen[0] +
                           cut_sz, rough_agpm_cen[1] -
                           cut_sz:rough_agpm_cen[1] + cut_sz]
    ...
    agpm_offset = [
            rough_agpm_offset[0] + psf_fit.x_mean - cut_sz,
            rough_agpm_offset[1] + psf_fit.y_mean - cut_sz
    ]
    agpm_pos = np.array(rough_agpm_cen) + [
            psf_fit.y_mean - cut_sz, psf_fit.x_mean - cut_sz
    ]
...
if rank == 0:
    ...
    for n in range(n_data):
        ...
        sys.stdout.write('\r\r\r AGPM offsets from ' + str(n + 1) +
                         ' received.')
        sys.stdout.flush()
    ...
    np.savetxt('all_agpm_offsets.txt', output_array, header=output_header,
               fmt='%.5e')
    ...
    print('X: ' + str(np.round(mean_offset[0], 4)) + ' +/- ' +
          str(np.round(std_offset[0], 4)))
    print('X: ' + str(np.round(mean_offset[1], 4)) + ' +/- ' +
          str(np.round(std_offset[1], 4)))

    np.savetxt(
            'agpm_offset.txt', mean_offset, header=
            '[X offset (pix), Y offset (pix)]. Mean offset between AGPM and the big circle taken from the sky sequence'
    )
    ...
    print(" Average time per cube: " +
          graphic_nompi_lib.humanize_time((MPI.Wtime() - t0) / (len(dirlist))) +
          " = " + str((MPI.Wtime() - t0) / (len(dirlist))) + " seconds.")
    sys.stdout.write('\n\n')
    sys.stdout.flush()
