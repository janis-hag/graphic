...
parser.add_argument('--centering_nsigma',action="store", dest="centering_nsigma", type=float, default=5,
                    help="Number of sigma used for rejection based on measured center position")
parser.add_argument('--flux_nsigma',action="store", dest="flux_nsigma", type=float, default=5,
                    help="Number of sigma used for rejection based on measured flux")
parser.add_argument('--psf_width_nsigma',action="store", dest="psf_width_nsigma", type=float, default=5,
                    help="Number of sigma used for rejection based on measured psf width")
...
parser.add_argument('-dithered', dest='dithered', action='store_const',const=True, default=False,
                    help='Switch for dithered data, so that centring rejection is performed cube-by-cube instead of globally.')
...
header_keys=['frame_number', 'psf_barycentre_x', 'psf_barycentre_y', 'psf_pixel_size',
             'psf_fit_centre_x', 'psf_fit_centre_y', 'psf_fit_height', 'psf_fit_width_x',
             'psf_fit_width_y', 'frame_num', 'frame_time', 'paralactic_angle']
...
if rank == 0 :
    ...
    if dithered:
        for cube_ix in range(ncubes):
            ...
            xcen_sigma=np.median(np.abs(xcens[cube_frames[cube_ix]]-np.median(xcens[cube_frames[cube_ix]])))
            ycen_sigma=np.median(np.abs(ycens[cube_frames[cube_ix]]-np.median(ycens[cube_frames[cube_ix]])))
            
            xcen_diff=np.abs(xcens[cube_frames[cube_ix]]-np.median(xcens[cube_frames[cube_ix]]))
            ycen_diff=np.abs(ycens[cube_frames[cube_ix]]-np.median(ycens[cube_frames[cube_ix]]))
                        
            valid_frames[cube_frames[cube_ix][xcen_diff > (centering_nsigma*xcen_sigma)]]=False
            valid_frames[cube_frames[cube_ix][ycen_diff > (centering_nsigma*ycen_sigma)]]=False
    else:        
    ...
    n_invalid[:,1] -= np.sum(n_invalid,axis=1) # first subtract the already known ones
    ...
    for cube_ix in range(ncubes):
        ...
        print "  Cube:",cube_ix,": ",np.sum(n_invalid[cube_ix])," invalid frames"
