...
parser = argparse.ArgumentParser(
        description=
        'Create a master flat from flatfields of different exposure time')
parser.add_argument('--pattern', action="store", dest="pattern",
                    default="SPHER*FLAT_FIELD",
                    help='cubes to use to create the master flat')
...
if rank == 0:
    ...
    def master_flat_sphere(key_word):
        ...
        hdr_masterflat[
                'Routine used'] = "master_flat_func routine from Sebastien P"  # Add a new keyword
        for allfiles in glob.iglob(key_word + '*'):
            print(allfiles)
            if len(allfiles) > 55:
                hdr_masterflat['flat nbr ' + np.str(count + 1) +
                               ' used'] = allfiles[:34]
            else:
                hdr_masterflat['flat nbr ' + np.str(count + 1) +
                               ' used'] = allfiles  # Add a new keyword
            if count == 0:
                ...
                if np.size(np.shape(temp)) > 2:
                    ...
                    temp = np.zeros(
                            ((1, np.shape(temp)[0], np.shape(temp)[1]))) + temp
                y = np.append(y, temp, axis=0)
                time_exp = np.append(time_exp, hdr["HIERARCH ESO DET SEQ1 DIT"])
            hdr_masterflat['Dit for flat nbr ' + np.str(count + 1)] = (
                    hdr["HIERARCH ESO DET SEQ1 DIT"], "s.")  # Add a new keyword
            count += 1

        shape_y = np.shape(y)
        x = np.append(np.ones((np.size(time_exp), 1)),
                      np.reshape(np.array(time_exp), (np.size(time_exp), 1)),
                      axis=1)
        ...
        pyfits.writeto("master_flat.fits", master_flat, header=hdr_masterflat,
