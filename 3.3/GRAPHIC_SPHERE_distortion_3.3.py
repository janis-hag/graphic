...
parser = argparse.ArgumentParser(
        description='Apply the correction of distortion only for IRDIS data cubes'
)
parser.add_argument('--pattern', action="store", dest="pattern",
                    default="cl_nomed",
                    help='cubes to apply the correction of distortion')
...
parser.add_argument(
        '--distortion_factor', action="store", dest="distortion_factor",
        default=1.006, type=float,
        help='Distortion factor to be applied to correct from distortion.')
parser.add_argument('-ifs', action="store_const", dest="ifs", const=True,
                    default=False, help='Switch for IFS data')
parser.add_argument(
        '--ifs_lowpass_r', action="store", dest="ifs_lowpass_r", default=5,
        type=float,
        help='R value for the Butterworth low pass filter applied to IFS data')
parser.add_argument(
        '--ifs_lowpass_cutoff', action="store", dest="ifs_lowpass_cutoff",
        default=120, type=float, help=
        'Cutoff value for the Butterworth low pass filter applied to IFS data')
parser.add_argument('--ifs_angle', action="store", dest="ifs_angle",
                    default=100.48, type=float,
                    help='Rotation between IFS and IRDIS field of view. ')
...
if rank == 0:
    ...
    for count, allfiles in enumerate(glob.iglob(pattern + '*')):
        ...
        if ifs:
            ...
            cube = graphic_nompi_lib.low_pass(cube, ifs_lowpass_r, 2,
                                              ifs_lowpass_cutoff)
            ...
            for ix, frame in enumerate(cube):
                derot_cube.append(
                        graphic_nompi_lib.fft_3shear_rotate_pad(
                                cube[ix], ifs_angle, pad=2, return_full=False))
        ...
        rescaled_cube = graphic_nompi_lib.rescale_image(cube,
                                                        correction_factor_x,
                                                        correction_factor_y)
        ...
        if ifs:
            ...
            for ix, frame in enumerate(rescaled_cube):
                derot_cube.append(
                        graphic_nompi_lib.fft_3shear_rotate_pad(
                                rescaled_cube[ix], -ifs_angle, pad=2,
                                return_full=False))
        ...
        # Write it out
        pyfits.writeto("dis_" + allfiles, rescaled_cube, header=hdr,
                       clobber=True)
