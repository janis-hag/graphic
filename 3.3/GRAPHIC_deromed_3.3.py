...
parser = argparse.ArgumentParser(
        description=
        'Derotates each single frame with respect to the parallactic angle.')
...
parser.add_argument('--naxis1', action="store", dest="naxis1", default=0,
                    type=int, help='Original frame shape (e.g. 1024)')
parser.add_argument('--steps', action="store", dest="steps", default=2,
                    type=int, help='Number of serialisation steps')
...
parser.add_argument('--info_type', action="store", dest="info_type",
                    default='rdb', help='Info directory')
parser.add_argument('--log_file', action="store", dest="log_file",
                    default='GRAPHIC', help='Log filename')
parser.add_argument('--pa_offset', action="store", dest="pa_offset",
                    default=0.0,
                    help='Offset to apply to the parallactic angle.')
parser.add_argument('-nomask', dest='nomask', action='store_const', const=False,
                    default=True,
                    help='Produce only one image with masked out centre.')
parser.add_argument(
        '--interpolate', dest='interpolate', action='store_const', const=True,
        default=False,
...
parser.add_argument('-mean', dest='use_mean', action='store_const', const=True,
                    default=False,
                    help='Use mean instead of median when combining frames.')
parser.add_argument(
        '-derotate_not_on_first_frame', dest='derotate_not_on_first_frame',
        action='store_const', const=True, default=False, help=
        'Derotate on the paralactic angle calculated and not on the first frame as it is done by default'
)
...
if rank == 0:
    ...
    cube_list, dirlist = graphic_nompi_lib.create_megatable(
            dirlist, infolist, keys=header_keys, nici=nici, sphere=sphere,
            scexao=scexao, fit=fit)

    if scexao:
        for cube_number in range(len(cube_list['info'])):
            for frame_number in range(cube_list['info'][cube_number].shape[0]):
                cube_list['info'][cube_number][frame_number][11] = -1 * float(
                        cube_list['info'][cube_number][frame_number][11])
    ...
    if derotate_not_on_first_frame:
        ...
        for cube_number in range(len(cube_list['info'])):
            for frame_number in range(cube_list['info'][cube_number].shape[0]):
                cube_list['info'][cube_number][frame_number][11] = float(
                        cube_list['info'][cube_number][frame_number]
                        [11]) + float(pa_offset)
                if p0 == -1:
    ...
    if 'ESO OBS TARG NAME' in hdr.keys():
        ...
        finalname = 'final_image_' + hdr['ESO OBS TARG NAME'].replace(
                ' ', '').upper() + '_' + fil + '_' + dirlist[0]
        ...
        finalname = 'final_image_' + hdr['OBJECT'].replace(
                ' ', '').upper() + '_' + fil + '_' + dirlist[0]
    else:
    ...
    for step in range(steps):
        ...
        if args.save_stack:
            graphic_nompi_lib.save_fits("derot_stack_" + step_filename, stack,
                                        hdr=hdr, backend='pyfits')

        print(' Step [' + str(step + 1) + '/' + str(steps) + '] of ' +
              combine_text + ' calculation')
        ...
        for p in range(nprocs - 1):
            ...
            if d > 0:
                print("Received " + combine_text + " reduced chunk from " +
                      str(p + 1))
        ...
        graphic_nompi_lib.save_fits(step_filename, med_tot, hdr=hdr,
                                    backend='pyfits')
        #Clean up
        if not prev_step_filename is None and os.access(prev_step_filename,
                                                        os.F_OK | os.R_OK):
            os.remove(prev_step_filename)
        prev_step_filename = step_filename
        print("Step time: " +
              graphic_nompi_lib.humanize_time((MPI.Wtime() - t0_step)))
    ...
    if "GC RMIN" in hdr.keys():
        ...
        fwhm = median(cube_list['info'][0][np.where(
                cube_list['info'][0][:, 7] > 0), 7])
    if nomask:
        med_nomask = med_tot.copy()
    med_tot = graphic_nompi_lib.mask_centre(med_tot, fwhm / 2.,
                                            med_tot.shape[0] / 2.,
                                            med_tot.shape[0] / 2.)
    ...
    if nomask:
        graphic_nompi_lib.save_fits('nomask_' + finalname, med_nomask, hdr=hdr,
                                    backend='pyfits')
    #Clean up
    if not prev_step_filename is None and os.access(prev_step_filename,
                                                    os.F_OK | os.R_OK):
        ...
        if todo == "derotate":
            ...
            for i in range(len(dirlist)):
                ...
                graphic_nompi_lib.iprint(
                        interactive, '\r\r\r ' + str(rank) +
                        ': Derotating cube ' + str(i + 1) + ' of ' +
                        str(len(dirlist)) + ' : ' + str(dirlist[i]) + '\n')

                hdulist_s = fits.open(dirlist[i], memmap=True)
                s_cube = hdulist_s[0].data
                rs_cube = np.ones((s_cube.shape[0], s_cube.shape[1],
                                   end - start)) * np.NaN
                ...
                for fn in range(s_cube.shape[0]):
                    if cube_list['info'][cn][fn,
                                             5] == -1:  # Invalid frame, skip
                        dprint(
                                d > 1,
                                'Skipping invalid frame, no PSf found: cube ' +
                                str(cn) + ', frame ' + str(fn))
                        s_cube = s_cube[1:]
                        continue
                    elif interpolate:
                        rs_cube[fn] = ndimage.interpolation.rotate(
                                s_cube[0], p0 - cube_list['info'][cn][fn, 11],
                                reshape=False, order=3, mode='constant',
                                cval=np.NaN, prefilter=False)[:, start:end]
                        s_cube = s_cube[1:]
                    ## elif not naxis1==0:
                    else:
                ...
                if d > 2:
                    graphic_nompi_lib.save_fits(
                            'rs_cube_' + str(rank) + '_' + dirlist[i], rs_cube,
                            hdr=hdulist_s[0].header, backend='pyfits')

                if full_stack is None:
                    full_stack = rs_cube.copy()
                else:
                    if full_stack.shape[1] == rs_cube.shape[
                            1] and full_stack.shape[2] == rs_cube.shape[2]:
                        full_stack = np.concatenate((full_stack, rs_cube),
                                                    axis=0)
                    else:
            ...
            graphic_nompi_lib.iprint(
                    interactive, "\r\r\r Process " + str(rank) + " of " +
                    str(nprocs - 1) + " calculating " + combine_text + ".")
