...
parser = argparse.ArgumentParser(
        description='Calculates astrometry of a companion using a grid search.')
parser.add_argument('--config', action="store", dest="config_file", type=str,
                    default=None)
...
calc_im_uncertainty = config.getboolean('functions', 'calc_im_uncertainty',
                                        fallback=False)
calc_psf_fwhm = config.getboolean('functions', 'calc_psf_fwhm', fallback=False)
test_likelihood = config.getboolean('functions', 'test_likelihood',
                                    fallback=False)
...
f_log.write('command graphic_astrometry_sphere_gridsearch.py run at ' +
...
if calc_im_uncertainty:
    ...
    pca_cube = pca.smart_annular_pca(
            cube, pca_settings['n_modes'], None, parangs_deg,
            n_fwhm=pca_settings['n_fwhm'], fwhm=pca_settings['fwhm'],
    ...
    comp_x = int(init_params[0] * np.sin(-init_params[1] * np.pi / 180.) +
                 final_im.shape[0] / 2)
    comp_y = int(init_params[0] * np.cos(-init_params[1] * np.pi / 180.) +
                 final_im.shape[0] / 2)
...
if test_likelihood:
    t0 = time.time()
    loglike = graphic_cfl.neg_fake_planet_loglikelihood(
            init_params, cube=cube, parangs_rad=parangs_rad, psf=psf,
            plot=False, uncert=image_uncert, pca_settings=pca_settings,
    ...
    f_log.write(
            'Est time for n_grid^3 iterations (current n_grid={}): {:.2f}hrs\n'.
...
if run_grid:
    ...
    seps = np.linspace(init_params[0] - range_params[0],
                       init_params[0] + range_params[0], num=n_grid)
    pas = np.linspace(init_params[1] - range_params[1],
                      init_params[1] + range_params[1], num=n_grid)
    cons = np.linspace(init_params[2] - range_params[2],
                       init_params[2] + range_params[2], num=n_grid)
    ...
    for sep_ix, sep in enumerate(seps):
        for pa_ix, pa in enumerate(pas):
            for con_ix, con in enumerate(cons):
                ...
                params = [sep, pa, con]
                loglike = graphic_cfl.neg_fake_planet_loglikelihood(
                        params, cube=cube, parangs_rad=parangs_rad, psf=psf,
                        plot=False, uncert=image_uncert,
                        pca_settings=pca_settings, fit_params=fit_params)

                grid_likes[sep_ix, pa_ix, con_ix] = np.exp(loglike)

    t1 = time.time()
    f_log.write('Time taken for grid: {0:.1f} mins for {1} gridpoints\n'.format(
            (t1 - t0) / 60, n_grid * n_grid * n_grid))
...
if plot_grid:
    ...
    for i in range(n_params):
        ...
        for j in range(i):
            ...
            plot_lims = [
                    params[i][0], params[i][-1], params[j][0], params[j][-1]
            ]
            plt.imshow(cond_like, extent=plot_lims, aspect='auto',
                       origin='lower')
...
if calc_values:
    ...
    for i in range(3):
        ...
        # Gaussian model
        gauss_model = models.Gaussian1D(amplitude=0.8, mean=init_params[i],
                                        stddev=range_params[i] / 2.)

        # LM fit
        fitter = LevMarLSQFitter()
        fit = fitter(
                gauss_model,
                np.linspace(init_params[i] - range_params[i],
                            init_params[i] + range_params[i], num=n_grid),
        ...
        if plot_grid:
            ...
            plt.subplot(n_params, n_params, n_params * i + i + 1)
            x_model = np.linspace(init_params[i] - range_params[i],
                                  init_params[i] + range_params[i], num=100)
            y_model = fit(x_model)
    ...
    PAs, sep_mas, _, _ = sphereastrometry.pa_IRDIS_seppa([store_pa, pa_err],
                                                         [store_sep, sep_err],
                                                         pscale)
    f.write('\n')
