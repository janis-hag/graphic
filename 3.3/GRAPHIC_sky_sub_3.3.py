...
parser.add_argument(
        ...
        help='Name of flat field to be used. If ' +
        'this argument is not set, the data will not be flat ' + 'fielded')
parser.add_argument(
        '--sky_interp', dest='sky_interp', action='store', type=int, default=1,
        help='Number of sky files to' +
...
if rank == 0:
    ...
    if not args.noinfo:
        if info_pattern == 'all_info':
            print('Warning, using default value: info_pattern=\"all_info\" ' +
                  'wrong info file may be used.')
...
for i in range(len(dirlist)):
    ...
    if sky_interp > 1:
        ...
        best_mjds = np.argsort(time_diffs)[
                0:sky_interp]  # this refers to the elements of sky_obstimes_arr
        ...
        if targ_mjd < current_mjds[0]:
            ...
            interp_funct = interp1d(current_mjds, current_skies,
                                    kind=(sky_interp - 1), axis=0)
    ...
    if args.normalise:
        for frame in range(cube.shape[0]):
            if all_info == 'empty' or not all_info[frame, 6] == -1:
                graphic_mpi_lib.dprint(
                        d > 1, 'Normalising factors: skymed=' + str(skymed) +
                        ' frame_median=' +
                        str(bottleneck.nanmedian(1. * cube[frame])))
                cube[frame] = cube[frame] - (bottleneck.nanmedian(
                        1. * cube[frame]) / skymed) * np.where(
                                np.isnan(sky), sky_med_frame, sky)
            else:
        ...
        graphic_mpi_lib.dprint(
                d > 2, "len(np.where(np.isnan(sky)[0])):" +
                str(len(np.where(np.isnan(sky))[0])))
    ...
    skyref = skyfile.split(os.sep)[
            -1]  # Strip directory away so we can record which sky file was used
    ...
    header['HIERARCH GC SKY_INTERP'] = (sky_interp,
                                        '# of sky frames used to interpolate')
    ...
    graphic_nompi_lib.save_fits(targetfile, cube.astype(np.float32), hdr=header,
                                backend='pyfits')
