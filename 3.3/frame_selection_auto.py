...
if rank==0:
	...
	def frame_selection(filename,r_int=30,r_ext=80,sigma=5):
	    ...
	    for i in range(np.shape(cube)[0]):
	        ...
	        index_quad_min=np.where(np.array([sum1[i],sum2[i],sum3[i],sum4[i]])
	        ...
	        # A frame is also bad if the median flux is sigma above the median absolute deviation
	...
	for i,allfiles in enumerate(glob.iglob(pattern+'*')):
	    bad_frames_cube[i]=frame_selection(allfiles,r_int=r_int,r_ext=r_ext,sigma=sigma)
