...
def subtract_principal_components(pcomps, image_region, mask=False):
    ...
    if (mask is False):
        ...
        transformed = np.dot(pcomps_windowed,
                             image_windowed.T)  #i.e. the coefficients
        reconstructed = np.dot(transformed.T, pcomps)
        residuals = image_region - reconstructed
...
def pca_multi(all_vars):
    ...
    # Get the principal components
    pcomps = principal_components(all_vars['cube_region'],
                                  n_modes=all_vars['n_modes'])
...
def pca_multi_annular(all_vars):
    ...
    for frame_ix, frame in enumerate(all_vars['cube_region']):
        # print('Frame: '+str(frame_ix)+'/'+str(len(all_vars['cube_region'])))
        ...
        # Get the principal components
        pcomps = principal_components(reference_frames,
                                      n_modes=all_vars['n_modes'])
        # Subtract them from the data
    ...
    if (region_ix % 5) == 4:
        ...
        time_left = (n_regions - region_ix -
                     1) * (time.time() - all_vars['t_start']) / (region_ix + 1)
        print('  Region ' + str(region_ix + 1) + ' done. Time remaining: ' +
              str(np.round(time_left / 60., 2)) + 'mins')
    return region_out


def pca_multi_annular_global():
    ''' A wrapper for the pca step, needed for multiprocessing. The parallelization
    is done over regions of the image, so each process needs access to the entire
    datacube for a small region only, and no processes need to share memory.
...
def find_reference_frames(parangs, radii, fwhm, n_fwhm, min_frames=None):
    ...
    for parang in parangs:
        ...
        for radius in radii:
            ...
            rot_dist = np.abs(2 * radius *
                              np.sin(delta_parang * np.pi / 180 / 2.))
...
def fft_rotate_multi(all_vars):
    ...
    return graphic_nompi_lib.fft_3shear_rotate_pad(
            all_vars['in_frame'], all_vars['alpha'], all_vars['pad'],
...
def define_annuli(npix, n_radii, arc_length, r_min, r_max='Default'):
    ...
    for radius_ix in range(n_radii):
        ...
        for azimuth_ix in range(n_azimuth):
            ...
            region_2d = np.where((pix_dist_map_2d >= minrad) &
                                 (pix_dist_map_2d < maxrad) &
                                 (azimuth_map_2d > minaz) &
                                 (azimuth_map_2d <= maxaz))

            # Ignore any regions that are partially outside the FoV and don't have many pixels
            if (minrad > npix / 2) and (len(
                    region_2d[0]) < (0.75 * arc_length * (maxrad - minrad))):
                continue
...
def simple_pca(image_file, n_modes, save_name, pc_name=None, output_dir='./'):
    ...
    if save_name:
        ...
        hdr = make_pca_header('simple_pca', n_modes, hdr=hdr,
    ...
    if pc_name:
        data = {
                'principal_components': pcomps.astype(np.float32),
                'regions': None,
                'npix': initial_shape[1],
...
def annular_pca(image_file, n_modes, save_name, n_annuli=5, arc_length=50,
                r_min=5, pc_name=None, threads=2, r_max='Default',
                output_dir='./'):
    ''' Performs a simple PCA reduction on the input datacube.
    ...
    regions, region_radii = define_annuli(initial_shape[1], n_annuli,
                                          arc_length, r_min=r_min, r_max=r_max)
    ...
    if (r_max < initial_shape[1] / 2.) and (r_max < initial_shape[2] / 2.):
        cube_out = cube_out[:,
                            np.int(cube_out.shape[1] // 2 -
                                   r_max):np.int(cube_out.shape[1] // 2 +
                                                 r_max),
                            np.int(cube_out.shape[2] // 2 -
                                   r_max):np.int(cube_out.shape[2] // 2 +
                                                 r_max)]

    if save_name:
        # Make a header to store the reduction parameters
        hdr = make_pca_header('annular_pca', n_modes, n_annuli=n_annuli,
                              arc_length=arc_length, hdr=hdr, r_min=r_min,
                              r_max=r_max, image_file=image_file)
...
def smart_pca(image_file, n_modes, save_name, parang_file, protection_angle=20,
    ...
    for ix, frame in enumerate(cube):
        ...
        if (ix % 10) == 9:
            time_left = (cube.shape[0] - ix - 1) * (time.time() -
                                                    t_start) / (ix + 1)
            print('  Done ' + str(ix + 1) + ' of ' + str(cube.shape[0]) + '. ' +
                  str(np.round(time_left / 60., 2)) + ' mins remaining')
    ...
    if save_name:
        # Make a header to store the reduction parameters
        hdr = make_pca_header('smart_pca', n_modes, hdr=hdr,
                              image_file=image_file)
...
def smart_annular_pca(image_file, n_modes, save_name, parang_file, n_fwhm=2,
    ...
    if not silent:
        print('  Minimum of: ' + str(np.min(n_good_frames)) +
              ' frames per annuli')
        print('  Using ' + str(n_modes) + ' modes, ' + str(n_fwhm) +
              ' FWHM protection angle,')
        print('  ' + str(fwhm) + ' pix FWHM, ' + str(n_annuli) + ' annuli, ' +
              str(arc_length) + ' pix arc length,')
        print('  minimum and maximum radii of ' + str(r_min) + ' and ' +
              str(r_max) + ' pix')
    ...
    if (np.min(n_good_frames) < n_modes) and not silent:
        ...
        if np.min(n_good_frames) == 0:
            ...
            min_rot = 2 * np.arcsin(n_fwhm * fwhm /
                                    (2 * np.min(region_radii))) * 180. / np.pi
            print('Minimum rotation needed for inner annuli: ' +
                  str(np.round(min_rot, decimals=1)))
            #            print('Found: '+str(np.round(np.max(parangs)-np.min(parangs),decimals=1))) # This doesnt work so dont bother printing it
            raise ValueError(
                    'Need at least 1 frame for PCA! Try decreasing n_fwhm or increasing min_reference_frames'
            )
    ...
    for region_ix, region in enumerate(regions):
        ...
        these_vars = {
                ...
                'n_modes': n_modes,
                'region': region,
                'all_good_frames': all_good_frames,
                't_start': time.time(),
                'cube_region': cube_region
        }
        all_vars.append(these_vars)
...
def smart_annular_pca_mpi(image_file, n_modes, save_name, parang_file, n_fwhm=2,
    ...
    if __name__ == 'pca':
        ...
        print('Pool with ' + str(threads) + ' threads started.')
        # Now let multiprocessing do it all
        pca_output = pool.map(pca_multi_annular, all_vars, chunksize=1)
        # Close the threads
        pool.close()
    else:
        print('Not main: ' + str(__name__))
...
def init_worker():
    import os
    print("In initializer pid is {} ppid is {}".format(os.getpid(),
                                                       os.getppid()))


def make_pca_header(pca_type, n_modes, hdr=None, n_fwhm='', fwhm='',
                    n_annuli='', arc_length='', r_min='', r_max='',
                    image_file='', min_reference_frames=''):
...
def classical_adi(image_file, save_name, parang_file, median=False,
                  silent=False, hdr=None, output_dir='./'):
    ...
    if save_name:
        hdr = make_pca_header('cADI', 1, hdr=hdr, image_file=image_file)
...
def smart_adi(image_file, save_name, parang_file, protection_angle=20,
              median=False, output_dir='./'):
...
def derotate_and_combine(image_file, parang_file, save_name='derot.fits',
    ...
    out_cube = np.zeros(
            (cube.shape[0], (cube.shape[1] + 2) * 2, (cube.shape[2] + 2) * 2))
    t_start = time.time()
    ...
    for ix in range(cube.shape[0]):
        in_frame = cube[ix]
        derot_frame = graphic_nompi_lib.fft_3shear_rotate_pad(
                in_frame, -parangs[ix], pad=2, return_full=True)
    ...
    xradius = out_frame.shape[0] // 2 - np.min([
            np.argmax(n_nans_x < out_frame.shape[0]),
            np.argmax(n_nans_x[::-1] < out_frame.shape[0])
    ])
    yradius = out_frame.shape[1] // 2 - np.min([
            np.argmax(n_nans_y < out_frame.shape[1]),
            np.argmax(n_nans_y[::-1] < out_frame.shape[1])
    ])
    ...
    out_cube = out_cube[:, out_frame.shape[0] // 2 -
                        xradius:out_frame.shape[0] // 2 + xradius,
                        out_frame.shape[1] // 2 -
                        yradius:out_frame.shape[1] // 2 + yradius]
    out_frame = out_frame[out_frame.shape[0] // 2 -
                          xradius:out_frame.shape[0] // 2 + xradius,
                          out_frame.shape[1] // 2 -
                          yradius:out_frame.shape[1] // 2 + yradius]
...
def derotate_and_combine_multi(image_file, parang_file, save_name='derot.fits',
                               ...
                               return_cube=True, hdr=None, silent=False,
                               output_dir='./'):
    '''Derotates and mean-combines a cube. Uses FFT derotation.
...
#
    ...
    out_cube = out_cube[:, out_frame.shape[0] // 2 -
                        xradius:out_frame.shape[0] // 2 + xradius,
                        out_frame.shape[1] // 2 -
                        xradius:out_frame.shape[1] // 2 + xradius]
    out_frame = out_frame[out_frame.shape[0] // 2 -
                          xradius:out_frame.shape[0] // 2 + xradius,
                          out_frame.shape[1] // 2 -
                          yradius:out_frame.shape[1] // 2 + yradius]
...
def crop_nans(cube, xradius=None):
    ...
    # The minimum of these is the one with the largest distance from the centre.
    # Then turn into distance from the centre by doing n_pixels/2-index
    xradius = cube.shape[-2] // 2 - np.min([
            np.argmax(n_nans_x < cube.shape[-2]),
            np.argmax(n_nans_x[::-1] < cube.shape[-2])
    ])
    yradius = cube.shape[-1] // 2 - np.min([
            np.argmax(n_nans_y < cube.shape[-1]),
            np.argmax(n_nans_y[::-1] < cube.shape[-1])
    ])
    ...
    if cube.ndim == 3:
        cube = cube[:,
                    cube.shape[-2] // 2 - xradius:cube.shape[-2] // 2 + xradius,
