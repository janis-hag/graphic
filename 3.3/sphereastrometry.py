...
def pa_IRDIS_seppa(pa, sep, pscale):
    ...
    **NOTE THAT the error on stellar position needs to already be incorporated into the 
    ...
    new_platescale_error = np.sqrt( (platescale[0]*anamorphic_error[1]/anamorphic_error[0])**2 + platescale[1]**2 )
    ...
    RA[1] = RA[0]*np.sqrt((sep_mas[1]/sep_mas[0])**2 + (PAs[1]/np.tan(PAs[0]))**2)

    dec = np.array([0.,0.])
    dec[0] = sep_mas[0]*np.cos(PAs[0])
    dec[1] = dec[0]*np.sqrt((sep_mas[1]/sep_mas[0])**2 + (PAs[1]*np.tan(PAs[0]))**2)
...
if __name__ == "__main__":
    ...
    PAs, sep_mas, RA, dec = pa_IRDIS_seppa([39.50,0.12],[43.66,0.11],'sph_h2')
    print(sep_mas, PAs)
    print('-----------------------------------------')
