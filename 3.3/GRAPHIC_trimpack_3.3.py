...
parser = argparse.ArgumentParser(
        description='Creates cubes by combining single frame files.')
...
parser.add_argument(
        '--naxis2', action="store", dest="naxis2", default=-1, type=int,
        help='The size of the single frames. No frame trimming if not set.')
parser.add_argument('--naxis3', action="store", dest="naxis3", default=4,
                    type=int, help='The number of frames per cube')
...
parser.add_argument('-collapse', dest='collapse', action='store_const',
                    const=True, default=False, help='Do not collapse cubes.')
...
parser.add_argument('-trimonly', dest='trimonly', action='store_const',
                    const=True, default=False, help='Trim only.')
...
header_keys = [
        'frame_number', 'psf_barycenter_x', 'psf_barycenter_y',
        'psf_pixel_size', 'psf_fit_center_x', 'psf_fit_center_y',
...
if args.trimonly:
    ...
    for i in range(len(dirlist)):
        ...
        if len(frame.shape) == 2:
            cube = frame[frame.shape[0] // 2 - naxis2 // 2:frame.shape[0] // 2 +
                         naxis2 // 2, frame.shape[1] // 2 -
                         naxis2 // 2:frame.shape[1] // 2 + naxis2 // 2]
            hdr['CRPIX1'] = '{0:14.7G}'.format(-frame.shape[0] // 2. +
                                               hdr['CRPIX1'] + naxis2 // 2.)
            hdr['CRPIX2'] = '{0:14.7G}'.format(-frame.shape[1] // 2. +
                                               hdr['CRPIX2'] + naxis2 // 2.)
        else:
            cube = frame[:, frame.shape[1] // 2 -
                         naxis2 // 2:frame.shape[1] // 2 + naxis2 // 2,
                         frame.shape[2] // 2 - naxis2 // 2:frame.shape[2] // 2 +
                         naxis2 // 2]
            hdr['CRPIX1'] = '{0:14.7G}'.format(-frame.shape[1] // 2. +
                                               hdr['CRPIX1'] + naxis2 // 2.)
            hdr['CRPIX2'] = '{0:14.7G}'.format(-frame.shape[2] // 2. +
                                               hdr['CRPIX2'] + naxis2 // 2.)

        hdr['HIERARCH GC TRIMPACK'] = str(__version__) + '.' + str(
                __subversion__)

        hdr['history'] = 'Updated CRPIX1, CRPIX2'

        trimpack_filename = target_pattern + "_" + dirlist[i]
        graphic_nompi_lib.save_fits(trimpack_filename, cube,
    ...
    for j in range(int(np.ceil(1. * len(dirlist) / naxis3))):
        ...
        if os.access(target_dir + os.sep + trimpack_filename,
                     os.F_OK | os.R_OK):
            print('Already processed: ' + trimpack_filename)
            skipped = skipped + 1
            continue
        # Check if already processed
        elif os.access(target_dir + os.sep + trimpack_filename + '.EMPTY',
                       os.F_OK | os.R_OK):
            print('Already processed, but no cube created: ' +
                  trimpack_filename)
            skipped = skipped + 1
            continue
        ...
        for n in range(n3):
            ...
            if c + n == len(dirlist):
                break
            sys.stdout.write("\n Processing cube [" + str(c + n + 1) + "/" +
                             str(len(dirlist)) + "]: " + str(dirlist[c + n]) +
                             "\n")
            sys.stdout.flush()
            ...
            if inst == 'sphere':
                ...
                if parang_list is None:
                    parang_list = np.array(
                            np.hstack((n,
                                       graphic_nompi_lib.create_parang_scexao(
                                               header))))
                    ## utcstart=datetime2jd(dateutil.parser.parse(hdr['DATE']+"T"+hdr['UT']))
                else:
                    parang_list = np.vstack(
                ...
                if parang_list is None:
                    parang_list = np.array(
                            np.hstack(
                                    (n,
                                     graphic_nompi_lib.create_parang_list_naco(
                                             header)[1:])))
                    ## utcstart=datetime2jd(dateutil.parser.parse(hdr['DATE']+"T"+hdr['UT']))
                else:
                    parang_list = np.vstack(
            ...
            # Trimming the frame
            cube[n] = frame[frame.shape[0] // 2 -
                            naxis2 // 2:frame.shape[0] // 2 + naxis2 // 2,
                            frame.shape[1] // 2 -
                            naxis2 // 2:frame.shape[1] // 2 + naxis2 // 2]
        ...
        sys.stdout.write(
                ...
                .format(
                        name=trimpack_filename,
                        human_time=graphic_nompi_lib.humanize_time(time.time() -
                                                                   t0_cube),
                        rate=os.path.getsize(trimpack_filename) /
                        (1048576 * (time.time() - t0_cube))))
        sys.stdout.write("Remaining time: " + graphic_nompi_lib.humanize_time(
                (time.time() - t_init) * (len(dirlist) - c) /
                (c - skipped + 1)) + "\n")
        sys.stdout.flush()
