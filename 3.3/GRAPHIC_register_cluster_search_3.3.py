...
parser.add_argument('--win', action="store", dest="win", type=float,
                    help='Gaussian fit window size')
parser.add_argument('--rmin', action="store", dest="rmin", type=float,
                    default=5, help='The minimal radius in pixels to consider')
parser.add_argument('--rmax', action="store", dest="rmax", type=float,
                    default=300, help='The Maximal spot size')
parser.add_argument('--t', action="store", dest="t", type=float, default=20,
                    help='The threshold value')
parser.add_argument('--t_max', action="store", dest="t_max", type=float,
                    help='The maximum deviation value')
parser.add_argument('--ratio', action="store", dest="ratio", type=float,
                    help='PSF ellipticity ratio')
...
parser.add_argument('--size', action="store", dest="size", type=int, default=-1,
                    help='Filter size')
parser.add_argument(
...
# No argument options...
parser.add_argument(
        '-deviation', dest='deviation', action='store_const', const=True,
        default=False, help=
        'Use a threshold based on standard deviation instead of an absolute threshold'
)
parser.add_argument('-sat', dest='nosat', action='store_const', const=False,
                    default=True, help='Discard saturated pixels')
parser.add_argument('--recurs', dest='recurs', action='store', type=int,
                    default=2500,
                    help='Change the recursion limit for the centroid search')
parser.add_argument('-no_neg', dest='no_neg', action='store_const', const=True,
                    default=False,
                    help='Null negative pixels before PSF fitting')
parser.add_argument('-moffat', dest='moffat', action='store_const', const=True,
                    default=False,
...
parser.add_argument(
        '-multispot', action='store_const', dest='multispot', const=True,
        default=False,
...
if rank == 0:  # Master process
    ...
    if len(dirlist) == 0 or ((spherepipe or chuck or scexao or naco_pack or
                              sphere_pack) and len(fctable_list) == 0):
    ...
    for i in range(len(dirlist)):
        ...
        ## else:
        filename = 'all_info_' + str(thres_coefficient) + "_" + str(
                min_size) + "_" + str(max_size) + "_" + dirlist[i][:-5] + '.rdb'
        ...
        if spherepipe:
            ...
            parang_list = np.atleast_2d(
                    graphic_nompi_lib.create_parang_list_sphere(cube_header))
        ...
        if 'ESO DET NDIT' in cube_header.keys() and not no_psf and cube_header[
                'NAXIS3'] == cube_header['ESO DET NDIT']:
            cent_list[-1] = -1

        # Set first frame to invalid for L_prime band due to cube reset effects
        if not no_psf and not args.keepfirst and 'ESO INS OPTI6 ID' in cube_header.keys(
    ...
    while not data_in is "over":
        if not data_in is None and isinstance(data_in, np.ndarray):
            for frame in range(data_in.shape[0]):
                print('[Rank ' + str(rank) + ', cube ' + str(cube_count) +
                      ']  Frame ' + str(frame + startframe) + ' of ' +
                      str(startframe + data_in.shape[0]))

                if window_size > 0:
                    data_in[frame] = data_in[
                            frame] - ndimage.filters.median_filter(
                                    data_in[frame], size=(window_size,
                                                          window_size),
                                    mode='reflect')
                if deviation:
                    ...
                    graphic_mpi_lib.dprint(
                            d > 1, "Sigma: " + str(sigma) + ", median: " +
                            str(median) + ", threshold: " + str(threshold))
                    data_in[frame] = data_in[frame] - median
                    data_in[frame] = np.where(
                            data_in[frame] > sigma * max_deviation, median,
                            data_in[frame])
                else:
                    threshold = thres_coefficient

                # Starting rough centre search and quality check
                cluster_array_ref, ref_ima, count = graphic_mpi_lib.cluster_search(
                        data_in[frame], threshold, min_size, max_size, x0_i,
                        y0_i, d=d)
                dprint(
                        d > 3, "cluster_search, on frame[" + str(frame) +
                        "]: " + str(cluster_array_ref))
                if not count == 1:  # Check if one and only one star has been found
                    if args.multispot and not cluster_array_ref is None:
                        ...
                        cluster_array_ref = np.append(cluster_array_ref, [
                                cluster_array_ref[0], cluster_array_ref[1],
                                cluster_array_ref[2], 1, 1
                        ])
                    else:
                    ...
                    if d == 1:
                        print('\r [Rank ' + str(rank) + ', cube ' +
                              str(cube_count) +
                              '] Number of spots detected in frame (' +
                              str(frame + 1) + '): ' + str(count) + '\n')

                    # Check if D needs to be redefined
                    if D + x0_i > data_in[frame].shape[0]:
                        if d > 2:
                            print("Redefined D because: D+x0_i>data_in[frame].shape[0] "
                                  + str(D) + "+" + str(x0_i) + "=" +
                                  str(D + x0_i) + ">" +
                                  str(data_in[frame].shape[0]))
                        D = data_in[frame].shape[0] - x0_i
                    elif D + y0_i > data_in[frame].shape[1]:
                        if d > 2:
                            print("Redefined D because: D+y0_i>data_in[frame].shape[1] "
                                  + str(D) + "+" + str(y0_i) + "=" +
                                  str(D + y0_i) + ">" +
                                  str(data_in[frame].shape[1]))
                        D = data_in[frame].shape[1] - y0_i
                    elif x0_i - D < 0:
                        if d > 2:
                            print("Redefined D because: D>y0_i " + str(D) +
                                  ">" + str(x0_i))
                        D = x0_i
                    elif y0_i - D < 0:
                        if d > 2:
                    ...
                    centre_win = data_in[frame, x0_i - D:x0_i + D,
                                         y0_i - D:y0_i + D]
                    ...
                    if d > 2:
                        print("")
                        print("height: " + str(g_param[0]) + " x, y: " +
                              str(g_param[1]) + ", " + str(g_param[2]) +
                              ", width_x, width_y: " + str(g_param[3]) + ", " +
                              str(g_param[4]))
                    ...
                    if moffat:
                        cluster_array_ref = np.append(cluster_array_ref, [
                                x0_g, y0_g, g_param[0], g_param[3], g_param[4]
                        ])
                    elif g_param[3] / g_param[4] < 1. / ratio and g_param[
                            3] / g_param[4] > ratio:
                        cluster_array_ref = np.append(cluster_array_ref, [
                                x0_g, y0_g, g_param[0], g_param[3], g_param[4]
                        ])
                        ...
                        print("\n PSF shape rejection: " +
                              str(g_param[3] / g_param[4]))
                        cluster_array_ref = np.append(cluster_array_ref,
                                                      [-1, -1, -1, -1, -1])

                cluster_array_ref = np.append(frame + startframe,
                                              cluster_array_ref)
