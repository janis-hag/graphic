...
parser.add_argument('--naxis3', action="store", dest="naxis3", required=True,
                    type=int, help='The number of frames to bin')
...
def read_recentre_cube(rcn, cube, rcube_list, l_max):
    ...
    if args.stat == True:
        print("\n STAT: Data upload took: " +
              graphic_lib.humanize_time(MPI.Wtime() - t0_trans))
        t0_trans = MPI.Wtime()
    ...
    for n in range(nprocs - 1):
        ...
        if data_in == None:
            continue
        sys.stdout.write('\r\r\r Recentreed data from ' + str(n + 1) +
                         ' received                                       =>')
        sys.stdout.flush()
...
if rank == 0:
    ...
    dirlist = graphic_lib.create_dirlist(pattern)

    ## infolist=glob.glob(info_dir+os.sep+'*'+info_pattern+'*.rdb')
    ## infolist.sort() # Sort the list alphabetically
    ##
    ##
    ## cube_list,dirlist=graphic_lib.create_megatable(dirlist,infolist,keys=header_keys,nici=nici,fit=fit)
    ...
    for c in range(len(dirlist)):  # Loop over the cubes
        ...
        if os.access(target_dir + os.sep + bin_filename, os.F_OK | os.R_OK):
            ...
            continue
        # Check if already processed
        elif os.access(target_dir + os.sep + bin_filename + '.EMPTY',
                       os.F_OK | os.R_OK):
        ...
        cube = cube * 1.
        new_cube = np.ones(
                (cube.shape[0] // naxis3, cube.shape[1], cube.shape[2])) * 1.

        sys.stdout.write("\n Processing cube [" + str(c + 1) + "/" +
        ...
        for i, j in enumerate(range(0, cube.shape[0], naxis3)):
            if j + naxis3 > cube.shape[0] or i + 1 == new_cube.shape[
                    0]:  # Taking all remaining frames
        ...
        hdr['HIERARCH GC BINNING'] = (str(__version__) + '.' + (__subversion__),
                                      "")
        hdr['HIERARCH GC FRAMES/BIN'] = (str(naxis3),
                                         "Number of franes in each bin")
        graphic_lib.save_fits(bin_filename, new_cube, target_dir=target_dir,
        ...
        sys.stdout.write(
                ...
                .format(
                        name=bin_filename,
                        human_time=graphic_lib.humanize_time(MPI.Wtime() -
                                                             t0_cube),
                        rate=os.path.getsize(bin_filename) /
                        (1048576 * (MPI.Wtime() - t0_cube))))
        sys.stdout.write("Remaining time: " +
                         graphic_lib.humanize_time((MPI.Wtime() - t_init) *
                                                   (len(dirlist) - c) /
                                                   (c - skipped + 1)) + "\n")
        sys.stdout.flush()
    ...
    print("Total time: " + graphic_lib.humanize_time((MPI.Wtime() - t_init)))
    ...
    if not hdr == None:
        if 'ESO OBS TARG NAME' in hdr.keys():
            log_file = log_file + "_" + string.replace(
                    hdr['ESO OBS TARG NAME'], ' ',
                    '') + "_" + str(__version__) + ".log"
        else:
            log_file = log_file + "_" + string.replace(
                    hdr['OBJECT'], ' ', '') + "_" + str(__version__) + ".log"
...
## bigstack[fn]=graphic_lib.fft_shift(bigstack[fn], stack.shape[1]/2.-info_stack[s+fn,4], stack.shape[2]/2.-info_stack[s+fn,5])
