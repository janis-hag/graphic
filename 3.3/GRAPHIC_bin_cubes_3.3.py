...
##
	...
	for c in range(len(dirlist)): # Loop over the cubes
		...
		graphic_lib.save_fits(bin_filename, new_cube, target_dir=target_dir,  hdr=hdr, backend='pyfits')

		sys.stdout.write("\n Saved: {name} .\n Processed in {human_time} at {rate:.2f} MB/s \n"
						 .format(name=bin_filename, human_time=graphic_lib.humanize_time(MPI.Wtime()-t0_cube) ,
								 rate=os.path.getsize(bin_filename)/(1048576*(MPI.Wtime()-t0_cube))))
		sys.stdout.write("Remaining time: "+graphic_lib.humanize_time((MPI.Wtime()-t_init)*(len(dirlist)-c)/(c-skipped+1))+"\n")
		sys.stdout.flush()
